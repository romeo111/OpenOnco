...
- BMA metadata consistency: ready=3, missing_primary=0, missing_evidence=0, missing_contra=0, guideline_order_mismatch=0
- BMA family groups: BIO-CTNNB1 x1, BIO-FGFR2 x1, BIO-PIK3R1 x1
- BMA review queue: intermediate_family_audit[medium] x3
...
| Check | Count | IDs |
|---|---:|---|
| Metadata-ready rows | 3 | BMA-CTNNB1-ENDOMETRIAL, BMA-FGFR2-MUTATION-ENDOMETRIAL, BMA-PIK3R1-ENDOMETRIAL |
...
```text
Switch to GPT-5.4-Mini medium. Use `py -3.12 -m scripts.normalized_actionability_coverage --inventory-disease DIS-ENDOMETRIAL --inventory-queue intermediate_family_audit --compact-json` as the only inventory input. Produce a source-grounded YAML edit plan only; do not author clinical claims.
```
