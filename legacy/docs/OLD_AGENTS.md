...
```
Workers (local LLM or Codex API) ◄──HTTP──► server.py (orchestrator)
                                               │
...
- `worker.py` — pulls jobs, runs research, submits results. Two modes: `--mode local` (Ollama via `local_llm.py`) or `--mode Codex` (requires `pip install anthropic` and `ANTHROPIC_API_KEY`). Simple cases (complexity ≤ 4) are routed to local workers; complex cases to Codex workers.
...
- `local_task_generator.py` — GPU-powered gap analysis. Runs local LLM over existing reports to identify fixable data gaps, then emits targeted Codex API re-eval prompts into `tasks/`.
...
- **`worker.py --mode Codex`**: `pip install anthropic`, needs `ANTHROPIC_API_KEY`.
...
(The old AGENTS.md claimed "no external dependencies" — that is only true for the core loop, not worker/Codex mode or fine-tuning.)
...
python worker.py --server http://<host>:8080 --mode Codex --max-jobs 10
```
