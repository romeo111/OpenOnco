...
class CaseEntry:
    ...
    label_en: str = ""
    summary_en: str = ""
    # Product metadata.  These values describe the scenario itself rather
    # than relying on a filename convention in the site builder.
    quality_tier: str = "curated"
    visibility: str = "public"
    scenario_type: str = "treatment"


# Scenarios kept for engineering regression work, but not published until
# their current profile produces the expected Plan or Diagnostic Brief.
# A build-time example audit enforces that this list never leaks to a public
# page.  Keep the backwards-compatible BROKEN_CASE_IDS alias below while
# callers migrate to the semantic visibility field on CaseEntry.
QUARANTINED_CASE_IDS: set[str] = {
    "auto-bcc",
    ...
    # The profile describes a real hereditary-risk question, but this KB
    # revision has no routable prevention indication for it yet.  Do not
    # publish a technical "no plan" page as a clinical example.
    "prevention-bap1-pedigree",
    # These two occupational profiles intentionally carry forward-compatible
    # findings while their dedicated prevention RedFlags are still being
    # authored.  Keep them available to internal QA, but never publish a
    # public example that produces no plan.
    "prevention-occ-chromium",
    "prevention-occ-nickel",
    # The profiles produce a plan, but the current KB does not yet select the
    # stated indication uniquely.  Do not present an ambiguous routing result
    # as a verified public treatment example.
    "verified-crc-l2-tucatinib-trastuzumab-crc",
    "verified-prostate-l2-cabazitaxel-mcrpc",
    "verified-soft-tissue-sarcoma-l2-gemcitabine-docetaxel-sts",
}

BROKEN_CASE_IDS: set[str] = QUARANTINED_CASE_IDS
...
CASES: list[CaseEntry] = [
    ...
    CaseEntry(
        case_id="bma-bma-alk-fusion-alcl",
        file="bma_bma_alk_fusion_alcl.json",
        label_ua="DIS-ALCL · BIO-ALK-FUSION (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-ALK-FUSION (NPM1-ALK and other ALK fusions). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="t_cell",
        label_en="DIS-ALCL · BIO-ALK-FUSION (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-ALK-FUSION (NPM1-ALK and other ALK fusions). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-atm-germline-breast",
        file="bma_bma_atm_germline_breast.json",
        label_ua="DIS-BREAST · BIO-HRR-PANEL (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-HRR-PANEL (ATM germline pathogenic). ESCAT IIA. Engine: 9 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-BREAST · BIO-HRR-PANEL (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-HRR-PANEL (ATM germline pathogenic). ESCAT IIA. Engine: 9 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-atm-germline-pdac",
        file="bma_bma_atm_germline_pdac.json",
        label_ua="DIS-PDAC · BIO-HRR-PANEL (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-HRR-PANEL (ATM germline pathogenic). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-PDAC · BIO-HRR-PANEL (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-HRR-PANEL (ATM germline pathogenic). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-atm-loss-cll",
        file="bma_bma_atm_loss_cll.json",
        label_ua="DIS-CLL · BIO-HRR-PANEL (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-HRR-PANEL (ATM loss (11q deletion or somatic mutation)). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_indolent",
        label_en="DIS-CLL · BIO-HRR-PANEL (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-HRR-PANEL (ATM loss (11q deletion or somatic mutation)). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-atm-loss-mcl",
        file="bma_bma_atm_loss_mcl.json",
        label_ua="DIS-MCL · BIO-HRR-PANEL (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-HRR-PANEL (ATM loss-of-function (mutation or 11q deletion)). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-MCL · BIO-HRR-PANEL (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-HRR-PANEL (ATM loss-of-function (mutation or 11q deletion)). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-axin2-germline-crc-oligodontia",
        file="bma_bma_axin2_germline_crc_oligodontia.json",
        label_ua="DIS-CRC · BIO-AXIN2-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-AXIN2-GERMLINE (AXIN2 germline pathogenic (oligodontia–colorectal cancer syn). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-AXIN2-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-AXIN2-GERMLINE (AXIN2 germline pathogenic (oligodontia–colorectal cancer syn). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-bard1-germline-ovarian",
        file="bma_bma_bard1_germline_ovarian.json",
        label_ua="DIS-OVARIAN · BIO-HRR-PANEL (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-HRR-PANEL (BARD1 germline pathogenic). ESCAT IIB. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-OVARIAN · BIO-HRR-PANEL (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-HRR-PANEL (BARD1 germline pathogenic). ESCAT IIB. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-bcl2-expression-aml",
        file="bma_bma_bcl2_expression_aml.json",
        label_ua="DIS-AML · BIO-BCL2-EXPRESSION-IHC (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-BCL2-EXPRESSION-IHC (BCL2 protein overexpression by IHC (H-score ≥100 or ≥50% cel). ESCAT IIB. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-AML · BIO-BCL2-EXPRESSION-IHC (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-BCL2-EXPRESSION-IHC (BCL2 protein overexpression by IHC (H-score ≥100 or ≥50% cel). ESCAT IIB. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-bcl2-expression-cll",
        file="bma_bma_bcl2_expression_cll.json",
        label_ua="DIS-CLL · BIO-BCL2-EXPRESSION-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-BCL2-EXPRESSION-IHC (BCL2 high expression (universal in CLL)). ESCAT IA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_indolent",
        label_en="DIS-CLL · BIO-BCL2-EXPRESSION-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-BCL2-EXPRESSION-IHC (BCL2 high expression (universal in CLL)). ESCAT IA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-bcl2-rearrangement-hgbl-dh",
        file="bma_bma_bcl2_rearrangement_hgbl_dh.json",
        label_ua="DIS-HGBL-DH · BIO-BCL2-REARRANGEMENT (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-BCL2-REARRANGEMENT (BCL2-R (component of double-hit; partner of MYC-R)). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-HGBL-DH · BIO-BCL2-REARRANGEMENT (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-BCL2-REARRANGEMENT (BCL2-R (component of double-hit; partner of MYC-R)). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-bcr-abl1-e255k-cml",
        file="bma_bma_bcr_abl1_e255k_cml.json",
        label_ua="DIS-CML · BIO-BCR-ABL1 (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-BCR-ABL1 (E255K / E255V (P-loop)). ESCAT IB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-CML · BIO-BCR-ABL1 (ESCAT IB)",
        summary_en="Synthetic profile: BIO-BCR-ABL1 (E255K / E255V (P-loop)). ESCAT IB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-bcr-abl1-f317l-ball",
        file="bma_bma_bcr_abl1_f317l_ball.json",
        label_ua="DIS-B-ALL · BIO-BCR-ABL1 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-BCR-ABL1 (F317L (acquired resistance)). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="lymphoblastic",
        label_en="DIS-B-ALL · BIO-BCR-ABL1 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-BCR-ABL1 (F317L (acquired resistance)). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-blm-germline-bloom-broad",
        file="bma_bma_blm_germline_bloom_broad.json",
        label_ua="DIS-CRC · BIO-BLM-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-BLM-GERMLINE (BLM germline biallelic pathogenic (Bloom syndrome — broad ca). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-BLM-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-BLM-GERMLINE (BLM germline biallelic pathogenic (Bloom syndrome — broad ca). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-braf-v600e-cholangio",
        file="bma_bma_braf_v600e_cholangio.json",
        label_ua="DIS-CHOLANGIOCARCINOMA · BIO-BRAF-V600E (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-BRAF-V600E (V600E (exon 15, kinase domain — present in ~5% of intrahepat). ESCAT IA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CHOLANGIOCARCINOMA · BIO-BRAF-V600E (ESCAT IA)",
        summary_en="Synthetic profile: BIO-BRAF-V600E (V600E (exon 15, kinase domain — present in ~5% of intrahepat). ESCAT IA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-braf-v600e-hcl",
        file="bma_bma_braf_v600e_hcl.json",
        label_ua="DIS-HCL · BIO-BRAF-V600E (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-BRAF-V600E (V600E). ESCAT IB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_indolent",
        label_en="DIS-HCL · BIO-BRAF-V600E (ESCAT IB)",
        summary_en="Synthetic profile: BIO-BRAF-V600E (V600E). ESCAT IB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-braf-v600e-ovarian",
        file="bma_bma_braf_v600e_ovarian.json",
        label_ua="DIS-OVARIAN · BIO-BRAF-V600E (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-BRAF-V600E (V600E). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-OVARIAN · BIO-BRAF-V600E (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-BRAF-V600E (V600E). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-braf-v600e-thyroid-anaplastic",
        file="bma_bma_braf_v600e_thyroid_anaplastic.json",
        label_ua="DIS-THYROID-ANAPLASTIC · BIO-BRAF-V600E (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-BRAF-V600E (V600E (exon 15, kinase domain — present in ~25-50% of anapla). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-THYROID-ANAPLASTIC · BIO-BRAF-V600E (ESCAT IA)",
        summary_en="Synthetic profile: BIO-BRAF-V600E (V600E (exon 15, kinase domain — present in ~25-50% of anapla). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-calr-et",
        file="bma_bma_calr_et.json",
        label_ua="DIS-ET · BIO-CALR (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-CALR (exon 9 indels — type-1/type-1-like (52-bp deletion) and type). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-ET · BIO-CALR (ESCAT IA)",
        summary_en="Synthetic profile: BIO-CALR (exon 9 indels — type-1/type-1-like (52-bp deletion) and type). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-calr-pmf",
        file="bma_bma_calr_pmf.json",
        label_ua="DIS-PMF · BIO-CALR (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-CALR (exon 9 indels — type-1/type-1-like (52-bp deletion) and type). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-PMF · BIO-CALR (ESCAT IA)",
        summary_en="Synthetic profile: BIO-CALR (exon 9 indels — type-1/type-1-like (52-bp deletion) and type). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ccnd1-ihc-mcl",
        file="bma_bma_ccnd1_ihc_mcl.json",
        label_ua="DIS-MCL · BIO-CCND1-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-CCND1-IHC (cyclin D1 IHC (universal in MCL; defines diagnosis with t(11). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-MCL · BIO-CCND1-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-CCND1-IHC (cyclin D1 IHC (universal in MCL; defines diagnosis with t(11). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ccnd1-ihc-mm",
        file="bma_bma_ccnd1_ihc_mm.json",
        label_ua="DIS-MM · BIO-CCND1-IHC (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-CCND1-IHC (CCND1 expression / cyclin D1 IHC (often surrogate for t(11;1). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloma",
        label_en="DIS-MM · BIO-CCND1-IHC (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-CCND1-IHC (CCND1 expression / cyclin D1 IHC (often surrogate for t(11;1). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ccnd1-t1114-mcl",
        file="bma_bma_ccnd1_t1114_mcl.json",
        label_ua="DIS-MCL · BIO-T11-14-IGH-CCND1 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-T11-14-IGH-CCND1 (t(11;14)(q13;q32) IGH/CCND1). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-MCL · BIO-T11-14-IGH-CCND1 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-T11-14-IGH-CCND1 (t(11;14)(q13;q32) IGH/CCND1). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ccnd1-t1114-mm",
        file="bma_bma_ccnd1_t1114_mm.json",
        label_ua="DIS-MM · BIO-T11-14-IGH-CCND1 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-T11-14-IGH-CCND1 (t(11;14)(q13;q32) IGH/CCND1). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloma",
        label_en="DIS-MM · BIO-T11-14-IGH-CCND1 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-T11-14-IGH-CCND1 (t(11;14)(q13;q32) IGH/CCND1). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-cd30-chl",
        file="bma_bma_cd30_chl.json",
        label_ua="DIS-CHL · BIO-CD30-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-CD30-IHC (CD30 expression on Reed-Sternberg / Hodgkin cells (~100% pos). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="hodgkin",
        label_en="DIS-CHL · BIO-CD30-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-CD30-IHC (CD30 expression on Reed-Sternberg / Hodgkin cells (~100% pos). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-cldn18-2-gastric",
        file="bma_bma_cldn18_2_gastric.json",
        label_ua="DIS-GASTRIC · BIO-CLDN18-2 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-CLDN18-2 (≥75% of tumor cells with moderate-to-strong (2+/3+) membrano). ESCAT IA. Engine: 5 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GASTRIC · BIO-CLDN18-2 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-CLDN18-2 (≥75% of tumor cells with moderate-to-strong (2+/3+) membrano). ESCAT IA. Engine: 5 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-csf1r-tgct",
        file="bma_bma_csf1r_tgct.json",
        label_ua="DIS-TGCT · BIO-CSF1R (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-CSF1R (CSF1R pathway activation — diffuse-type TGCT (COL6A3-CSF1 fu). ESCAT IA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-TGCT · BIO-CSF1R (ESCAT IA)",
        summary_en="Synthetic profile: BIO-CSF1R (CSF1R pathway activation — diffuse-type TGCT (COL6A3-CSF1 fu). ESCAT IA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ctnnb1-endometrial",
        file="bma_bma_ctnnb1_endometrial.json",
        label_ua="DIS-ENDOMETRIAL · BIO-CTNNB1 (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-CTNNB1 (CTNNB1 exon 3 activating mutation (S33C, S37F, T41A, S45P) i). ESCAT IIB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-ENDOMETRIAL · BIO-CTNNB1 (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-CTNNB1 (CTNNB1 exon 3 activating mutation (S33C, S37F, T41A, S45P) i). ESCAT IIB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-dicer1-germline-pleuropulmonary-blastoma",
        file="bma_bma_dicer1_germline_pleuropulmonary_blastoma.json",
        label_ua="DIS-OVARIAN · BIO-DICER1-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-DICER1-GERMLINE (DICER1 germline pathogenic (RNase IIIb domain hotspot or LoF). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-OVARIAN · BIO-DICER1-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-DICER1-GERMLINE (DICER1 germline pathogenic (RNase IIIb domain hotspot or LoF). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-epcam-germline-crc",
        file="bma_bma_epcam_germline_crc.json",
        label_ua="DIS-CRC · BIO-DMMR-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-DMMR-IHC (EPCAM germline 3' deletion (silences MSH2 by promoter methyl). ESCAT IA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-DMMR-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-DMMR-IHC (EPCAM germline 3' deletion (silences MSH2 by promoter methyl). ESCAT IA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ezh2-y641-fl",
        file="bma_bma_ezh2_y641_fl.json",
        label_ua="DIS-FL · BIO-EZH2-Y641 (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-EZH2-Y641 (Y641N/F/H/S/C activating mutations in SET domain — gain-of-f). ESCAT IB. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_indolent",
        label_en="DIS-FL · BIO-EZH2-Y641 (ESCAT IB)",
        summary_en="Synthetic profile: BIO-EZH2-Y641 (Y641N/F/H/S/C activating mutations in SET domain — gain-of-f). ESCAT IB. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-fgfr2-amp-gastric",
        file="bma_bma_fgfr2_amp_gastric.json",
        label_ua="DIS-GASTRIC · BIO-FGFR2 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-FGFR2 (amplification (~5-7% gastric/GEJ; high-level GCN ≥10)). ESCAT IIA. Engine: 5 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GASTRIC · BIO-FGFR2 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-FGFR2 (amplification (~5-7% gastric/GEJ; high-level GCN ≥10)). ESCAT IIA. Engine: 5 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-fgfr2-bicc1-cholangio",
        file="bma_bma_fgfr2_bicc1_cholangio.json",
        label_ua="DIS-CHOLANGIOCARCINOMA · BIO-FGFR2 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-FGFR2 (BICC1-FGFR2 fusion (most common partner ~30-40% of FGFR2-fus). ESCAT IA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CHOLANGIOCARCINOMA · BIO-FGFR2 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-FGFR2 (BICC1-FGFR2 fusion (most common partner ~30-40% of FGFR2-fus). ESCAT IA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-fgfr2-mutation-endometrial",
        file="bma_bma_fgfr2_mutation_endometrial.json",
        label_ua="DIS-ENDOMETRIAL · BIO-FGFR2 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-FGFR2 (activating mutation (~12% endometrioid; hotspots S252W, N549). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-ENDOMETRIAL · BIO-FGFR2 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-FGFR2 (activating mutation (~12% endometrioid; hotspots S252W, N549). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-fgfr2-mutation-urothelial",
        file="bma_bma_fgfr2_mutation_urothelial.json",
        label_ua="DIS-UROTHELIAL · BIO-FGFR2 (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-FGFR2 (activating mutation (FGFR2 fusion or kinase-domain mutation)). ESCAT IB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-UROTHELIAL · BIO-FGFR2 (ESCAT IB)",
        summary_en="Synthetic profile: BIO-FGFR2 (activating mutation (FGFR2 fusion or kinase-domain mutation)). ESCAT IB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-fgfr2b-membrane-gastric",
        file="bma_bma_fgfr2b_membrane_gastric.json",
        label_ua="DIS-GASTRIC · BIO-FGFR2B-IHC (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-FGFR2B-IHC (FGFR2b 2+/3+ membranous staining in ≥10% of tumor cells by I). ESCAT IIA. Engine: 5 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GASTRIC · BIO-FGFR2B-IHC (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-FGFR2B-IHC (FGFR2b 2+/3+ membranous staining in ≥10% of tumor cells by I). ESCAT IIA. Engine: 5 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-fgfr3-r248c-urothelial",
        file="bma_bma_fgfr3_r248c_urothelial.json",
        label_ua="DIS-UROTHELIAL · BIO-FGFR3-MUTATION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-FGFR3-MUTATION (R248C). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-UROTHELIAL · BIO-FGFR3-MUTATION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-FGFR3-MUTATION (R248C). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-flt3-d835-aml",
        file="bma_bma_flt3_d835_aml.json",
        label_ua="DIS-AML · BIO-FLT3-D835 (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-FLT3-D835 (TKD point mutation (D835Y/V/H/F; ~7% AML, often co-occurs wi). ESCAT IB. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-AML · BIO-FLT3-D835 (ESCAT IB)",
        summary_en="Synthetic profile: BIO-FLT3-D835 (TKD point mutation (D835Y/V/H/F; ~7% AML, often co-occurs wi). ESCAT IB. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-flt3-itd-aml",
        file="bma_bma_flt3_itd_aml.json",
        label_ua="DIS-AML · BIO-FLT3-ITD (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-FLT3-ITD (internal tandem duplication (juxtamembrane domain — ~25% of ). ESCAT IA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-AML · BIO-FLT3-ITD (ESCAT IA)",
        summary_en="Synthetic profile: BIO-FLT3-ITD (internal tandem duplication (juxtamembrane domain — ~25% of ). ESCAT IA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-foxl2-granulosa-cell",
        file="bma_bma_foxl2_granulosa_cell.json",
        label_ua="DIS-GRANULOSA-CELL · BIO-FOXL2 (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-FOXL2 (FOXL2 p.C134W (c.402C>G) — present in ~97% of adult granulos). ESCAT IIB. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GRANULOSA-CELL · BIO-FOXL2 (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-FOXL2 (FOXL2 p.C134W (c.402C>G) — present in ~97% of adult granulos). ESCAT IIB. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-gnas-ipmn",
        file="bma_bma_gnas_ipmn.json",
        label_ua="DIS-PDAC · BIO-GNAS (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-GNAS (GNAS R201 activating mutation (R201C, R201H) in pancreatic c). ESCAT IIB. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-PDAC · BIO-GNAS (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-GNAS (GNAS R201 activating mutation (R201C, R201H) in pancreatic c). ESCAT IIB. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-her2-amp-cholangio",
        file="bma_bma_her2_amp_cholangio.json",
        label_ua="DIS-CHOLANGIOCARCINOMA · BIO-HER2-SOLID (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-HER2-SOLID (amplification / overexpression — IHC 3+ or (IHC 2+ + ISH amp). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CHOLANGIOCARCINOMA · BIO-HER2-SOLID (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-HER2-SOLID (amplification / overexpression — IHC 3+ or (IHC 2+ + ISH amp). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-her2-amp-crc",
        file="bma_bma_her2_amp_crc.json",
        label_ua="DIS-CRC · BIO-HER2-SOLID (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-HER2-SOLID (amplification / overexpression — IHC 3+ or (IHC 2+ + ISH amp). ESCAT IB. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-HER2-SOLID (ESCAT IB)",
        summary_en="Synthetic profile: BIO-HER2-SOLID (amplification / overexpression — IHC 3+ or (IHC 2+ + ISH amp). ESCAT IB. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-her2-low-breast",
        file="bma_bma_her2_low_breast.json",
        label_ua="DIS-BREAST · BIO-HER2-LOW (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-HER2-LOW (IHC 1+ OR IHC 2+/ISH-negative (HER2-low by ASCO/CAP 2023 cri). ESCAT IA. Engine: 9 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-BREAST · BIO-HER2-LOW (ESCAT IA)",
        summary_en="Synthetic profile: BIO-HER2-LOW (IHC 1+ OR IHC 2+/ISH-negative (HER2-low by ASCO/CAP 2023 cri). ESCAT IA. Engine: 9 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-hnf1a-hcc",
        file="bma_bma_hnf1a_hcc.json",
        label_ua="DIS-HCC · BIO-HNF1A (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-HNF1A (HNF1A somatic mutation in hepatocellular carcinoma and hepat). ESCAT IIB. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-HCC · BIO-HNF1A (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-HNF1A (HNF1A somatic mutation in hepatocellular carcinoma and hepat). ESCAT IIB. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-hrd-status-breast",
        file="bma_bma_hrd_status_breast.json",
        label_ua="DIS-BREAST · BIO-HRD-STATUS (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-HRD-STATUS (germline BRCA1/2 (gBRCA) — primary FDA companion biomarker; ). ESCAT IA. Engine: 9 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-BREAST · BIO-HRD-STATUS (ESCAT IA)",
        summary_en="Synthetic profile: BIO-HRD-STATUS (germline BRCA1/2 (gBRCA) — primary FDA companion biomarker; ). ESCAT IA. Engine: 9 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-hrd-status-pdac",
        file="bma_bma_hrd_status_pdac.json",
        label_ua="DIS-PDAC · BIO-HRD-STATUS (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-HRD-STATUS (germline BRCA1/2 (gBRCAm) — primary FDA-actionable subset (~). ESCAT IA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-PDAC · BIO-HRD-STATUS (ESCAT IA)",
        summary_en="Synthetic profile: BIO-HRD-STATUS (germline BRCA1/2 (gBRCAm) — primary FDA-actionable subset (~). ESCAT IA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-hrd-status-prostate",
        file="bma_bma_hrd_status_prostate.json",
        label_ua="DIS-PROSTATE · BIO-HRD-STATUS (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-HRD-STATUS (HRR-pathway alteration — BRCA1/2 (~10% of mCRPC), ATM, CHEK2). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-PROSTATE · BIO-HRD-STATUS (ESCAT IA)",
        summary_en="Synthetic profile: BIO-HRD-STATUS (HRR-pathway alteration — BRCA1/2 (~10% of mCRPC), ATM, CHEK2). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-idh1-r132-cholangio",
        file="bma_bma_idh1_r132_cholangio.json",
        label_ua="DIS-CHOLANGIOCARCINOMA · BIO-IDH-MUTATION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-IDH-MUTATION (IDH1 R132 hotspot (R132C ~70%, R132L, R132G, R132H, R132S; ~). ESCAT IA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CHOLANGIOCARCINOMA · BIO-IDH-MUTATION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-IDH-MUTATION (IDH1 R132 hotspot (R132C ~70%, R132L, R132G, R132H, R132S; ~). ESCAT IA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-idh1-r132c-aml",
        file="bma_bma_idh1_r132c_aml.json",
        label_ua="DIS-AML · BIO-IDH-MUTATION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-IDH-MUTATION (IDH1 R132C). ESCAT IA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-AML · BIO-IDH-MUTATION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-IDH-MUTATION (IDH1 R132C). ESCAT IA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-idh1-r132h-mds-hr",
        file="bma_bma_idh1_r132h_mds_hr.json",
        label_ua="DIS-MDS-HR · BIO-IDH-MUTATION (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-IDH-MUTATION (IDH1 R132H). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-MDS-HR · BIO-IDH-MUTATION (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-IDH-MUTATION (IDH1 R132H). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-idh1-r132h-mds-lr",
        file="bma_bma_idh1_r132h_mds_lr.json",
        label_ua="DIS-MDS-LR · BIO-IDH-MUTATION (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-IDH-MUTATION (IDH1 R132H). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-MDS-LR · BIO-IDH-MUTATION (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-IDH-MUTATION (IDH1 R132H). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-idh-mutation-glioma-low-grade",
        file="bma_bma_idh_mutation_glioma_low_grade.json",
        label_ua="DIS-GLIOMA-LOW-GRADE · BIO-IDH-MUTATION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-IDH-MUTATION (IDH1 or IDH2 mutation (IDH1 R132H most common >90%; IDH1 R13). ESCAT IA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GLIOMA-LOW-GRADE · BIO-IDH-MUTATION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-IDH-MUTATION (IDH1 or IDH2 mutation (IDH1 R132H most common >90%; IDH1 R13). ESCAT IA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ighv-unmutated-cll",
        file="bma_bma_ighv_unmutated_cll.json",
        label_ua="DIS-CLL · BIO-IGHV-MUTATIONAL-STATUS (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-IGHV-MUTATIONAL-STATUS (IGHV-unmutated (≥98% homology to germline) — adverse risk; ~). ESCAT IA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_indolent",
        label_en="DIS-CLL · BIO-IGHV-MUTATIONAL-STATUS (ESCAT IA)",
        summary_en="Synthetic profile: BIO-IGHV-MUTATIONAL-STATUS (IGHV-unmutated (≥98% homology to germline) — adverse risk; ~). ESCAT IA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-jak2-v617f-et",
        file="bma_bma_jak2_v617f_et.json",
        label_ua="DIS-ET · BIO-JAK2 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-JAK2 (V617F (exon 14 — present in ~50-60% of essential thrombocyth). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-ET · BIO-JAK2 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-JAK2 (V617F (exon 14 — present in ~50-60% of essential thrombocyth). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-jak2-v617f-pmf",
        file="bma_bma_jak2_v617f_pmf.json",
        label_ua="DIS-PMF · BIO-JAK2 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-JAK2 (V617F (exon 14 — present in ~50-60% of primary myelofibrosis). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-PMF · BIO-JAK2 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-JAK2 (V617F (exon 14 — present in ~50-60% of primary myelofibrosis). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-jak2-v617f-pv",
        file="bma_bma_jak2_v617f_pv.json",
        label_ua="DIS-PV · BIO-JAK2 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-JAK2 (V617F (exon 14, JH2 pseudokinase domain — present in ~95% of). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-PV · BIO-JAK2 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-JAK2 (V617F (exon 14, JH2 pseudokinase domain — present in ~95% of). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-jak3-atll",
        file="bma_bma_jak3_atll.json",
        label_ua="DIS-ATLL · BIO-JAK3 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-JAK3 (JAK3 activating mutation (A572V, A573V, M511I most common) —). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="t_cell",
        label_en="DIS-ATLL · BIO-JAK3 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-JAK3 (JAK3 activating mutation (A572V, A573V, M511I most common) —). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-jak3-ctcl",
        file="bma_bma_jak3_ctcl.json",
        label_ua="DIS-MF-SEZARY · BIO-JAK3 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-JAK3 (JAK3 activating mutation — mycosis fungoides (MF) or Sézary ). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="t_cell",
        label_en="DIS-MF-SEZARY · BIO-JAK3 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-JAK3 (JAK3 activating mutation — mycosis fungoides (MF) or Sézary ). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-kit-d816v-gist",
        file="bma_bma_kit_d816v_gist.json",
        label_ua="DIS-GIST · BIO-KIT (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-KIT (D816V (activation loop, exon 17 — rare in GIST primary; seco). ESCAT IIA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GIST · BIO-KIT (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-KIT (D816V (activation loop, exon 17 — rare in GIST primary; seco). ESCAT IIA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-kit-mutation-aml",
        file="bma_bma_kit_mutation_aml.json",
        label_ua="DIS-AML · BIO-KIT (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-KIT (activating mutation (exon 17 D816V / N822K most common; ~25-). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-AML · BIO-KIT (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-KIT (activating mutation (exon 17 D816V / N822K most common; ~25-). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-kmt2a-rearr-aml",
        file="bma_bma_kmt2a_rearr_aml.json",
        label_ua="DIS-AML · BIO-KMT2A-REARRANGEMENT (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-KMT2A-REARRANGEMENT (KMT2A (MLL) rearrangement — any fusion partner (KMT2A-MLLT3/). ESCAT IA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-AML · BIO-KMT2A-REARRANGEMENT (ESCAT IA)",
        summary_en="Synthetic profile: BIO-KMT2A-REARRANGEMENT (KMT2A (MLL) rearrangement — any fusion partner (KMT2A-MLLT3/). ESCAT IA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-kmt2a-rearr-b-all",
        file="bma_bma_kmt2a_rearr_b_all.json",
        label_ua="DIS-B-ALL · BIO-KMT2A-REARRANGEMENT (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-KMT2A-REARRANGEMENT (KMT2A (MLL) rearrangement — any fusion partner; infant ALL (). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="lymphoblastic",
        label_en="DIS-B-ALL · BIO-KMT2A-REARRANGEMENT (ESCAT IA)",
        summary_en="Synthetic profile: BIO-KMT2A-REARRANGEMENT (KMT2A (MLL) rearrangement — any fusion partner; infant ALL (). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-kras-g12c-crc",
        file="bma_bma_kras_g12c_crc.json",
        label_ua="DIS-CRC · BIO-KRAS-G12C (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-KRAS-G12C (G12C). ESCAT IB. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-KRAS-G12C (ESCAT IB)",
        summary_en="Synthetic profile: BIO-KRAS-G12C (G12C). ESCAT IB. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-kras-g12d-pdac",
        file="bma_bma_kras_g12d_pdac.json",
        label_ua="DIS-PDAC · BIO-RAS-MUTATION (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-RAS-MUTATION (KRAS G12D). ESCAT IIB. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-PDAC · BIO-RAS-MUTATION (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-RAS-MUTATION (KRAS G12D). ESCAT IIB. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-kras-q61-melanoma",
        file="bma_bma_kras_q61_melanoma.json",
        label_ua="DIS-MELANOMA · BIO-RAS-MUTATION (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-RAS-MUTATION (KRAS Q61X). ESCAT IIB. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-MELANOMA · BIO-RAS-MUTATION (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-RAS-MUTATION (KRAS Q61X). ESCAT IIB. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-map2k1-hcl",
        file="bma_bma_map2k1_hcl.json",
        label_ua="DIS-HCL · BIO-MAP2K1 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-MAP2K1 (MAP2K1 activating mutation (K57E, Q56P, E203K) — hairy cell ). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_indolent",
        label_en="DIS-HCL · BIO-MAP2K1 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-MAP2K1 (MAP2K1 activating mutation (K57E, Q56P, E203K) — hairy cell ). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-map2k1-lgg",
        file="bma_bma_map2k1_lgg.json",
        label_ua="DIS-GLIOMA-LOW-GRADE · BIO-MAP2K1 (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-MAP2K1 (MAP2K1 activating mutation — low-grade glioma (LGG); BRAF-fu). ESCAT IIA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GLIOMA-LOW-GRADE · BIO-MAP2K1 (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-MAP2K1 (MAP2K1 activating mutation — low-grade glioma (LGG); BRAF-fu). ESCAT IIA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        ...
        summary_ua="Синтетичний профіль: BIO-MET (amplification or activating mutation (papillary type-1 conte). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-RCC · BIO-MET (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-MET (amplification or activating mutation (papillary type-1 conte). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-mlh1-germline-gastric",
        file="bma_bma_mlh1_germline_gastric.json",
        label_ua="DIS-GASTRIC · BIO-DMMR-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 5 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GASTRIC · BIO-DMMR-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 5 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-mlh1-germline-ovarian",
        file="bma_bma_mlh1_germline_ovarian.json",
        label_ua="DIS-OVARIAN · BIO-DMMR-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-OVARIAN · BIO-DMMR-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-mlh1-germline-prostate",
        file="bma_bma_mlh1_germline_prostate.json",
        label_ua="DIS-PROSTATE · BIO-DMMR-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-PROSTATE · BIO-DMMR-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-mlh1-germline-urothelial",
        file="bma_bma_mlh1_germline_urothelial.json",
        label_ua="DIS-UROTHELIAL · BIO-DMMR-IHC (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-UROTHELIAL · BIO-DMMR-IHC (ESCAT IA)",
        summary_en="Synthetic profile: BIO-DMMR-IHC (MLH1 germline loss-of-function (dMMR / MSI-H)). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-msh3-germline-polyposis-biallelic",
        file="bma_bma_msh3_germline_polyposis_biallelic.json",
        label_ua="DIS-CRC · BIO-MSH3-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-MSH3-GERMLINE (MSH3 germline biallelic pathogenic (MSH3-associated polyposi). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-MSH3-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-MSH3-GERMLINE (MSH3 germline biallelic pathogenic (MSH3-associated polyposi). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-myc-rearrangement-burkitt",
        file="bma_bma_myc_rearrangement_burkitt.json",
        label_ua="DIS-BURKITT · BIO-MYC-REARRANGEMENT (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-MYC-REARRANGEMENT (t(8;14) IGH/MYC (or t(2;8), t(8;22) — variant)). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-BURKITT · BIO-MYC-REARRANGEMENT (ESCAT IA)",
        summary_en="Synthetic profile: BIO-MYC-REARRANGEMENT (t(8;14) IGH/MYC (or t(2;8), t(8;22) — variant)). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-myd88-l265p-dlbcl-nos",
        file="bma_bma_myd88_l265p_dlbcl_nos.json",
        label_ua="DIS-DLBCL-NOS · BIO-MYD88-L265P (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-MYD88-L265P (L265P (often with CD79B mutation — MCD/C5 cluster)). ESCAT IIB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-DLBCL-NOS · BIO-MYD88-L265P (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-MYD88-L265P (L265P (often with CD79B mutation — MCD/C5 cluster)). ESCAT IIB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-myd88-l265p-pcnsl",
        file="bma_bma_myd88_l265p_pcnsl.json",
        label_ua="DIS-PCNSL · BIO-MYD88-L265P (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-MYD88-L265P (L265P). ESCAT IIB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-PCNSL · BIO-MYD88-L265P (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-MYD88-L265P (L265P). ESCAT IIB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-nf2-germline-vestibular-schwannoma",
        file="bma_bma_nf2_germline_vestibular_schwannoma.json",
        label_ua="DIS-MENINGIOMA · BIO-NF2-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-NF2-GERMLINE (NF2 germline pathogenic (merlin/schwannomin loss-of-function). ESCAT IIA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-MENINGIOMA · BIO-NF2-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-NF2-GERMLINE (NF2 germline pathogenic (merlin/schwannomin loss-of-function). ESCAT IIA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-npm1-aml",
        file="bma_bma_npm1_aml.json",
        label_ua="DIS-AML · BIO-NPM1 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-NPM1 (exon 12 frameshift insertions (type A 4-bp duplication most ). ESCAT IA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloid",
        label_en="DIS-AML · BIO-NPM1 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-NPM1 (exon 12 frameshift insertions (type A 4-bp duplication most ). ESCAT IA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-nrg1-fusion-nsclc",
        file="bma_bma_nrg1_fusion_nsclc.json",
        label_ua="DIS-NSCLC · BIO-NRG1-FUSION (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-NRG1-FUSION (NRG1 gene fusion — any fusion partner (CD74-NRG1 most common). ESCAT IIA. Engine: 10 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-NSCLC · BIO-NRG1-FUSION (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-NRG1-FUSION (NRG1 gene fusion — any fusion partner (CD74-NRG1 most common). ESCAT IIA. Engine: 10 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-nrg1-fusion-pdac",
        file="bma_bma_nrg1_fusion_pdac.json",
        label_ua="DIS-PDAC · BIO-NRG1-FUSION (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-NRG1-FUSION (NRG1 gene fusion — any fusion partner (RBPMS-NRG1, SLC3A2-NR). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-PDAC · BIO-NRG1-FUSION (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-NRG1-FUSION (NRG1 gene fusion — any fusion partner (RBPMS-NRG1, SLC3A2-NR). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-nthl1-germline-nap-biallelic",
        file="bma_bma_nthl1_germline_nap_biallelic.json",
        label_ua="DIS-CRC · BIO-NTHL1-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-NTHL1-GERMLINE (NTHL1 germline biallelic pathogenic (NTHL1-associated polypo). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-NTHL1-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-NTHL1-GERMLINE (NTHL1 germline biallelic pathogenic (NTHL1-associated polypo). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ntrk-etv6-salivary",
        file="bma_bma_ntrk_etv6_salivary.json",
        label_ua="DIS-SALIVARY · BIO-NTRK-FUSION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-NTRK-FUSION (ETV6-NTRK3 fusion). ESCAT IA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-SALIVARY · BIO-NTRK-FUSION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-NTRK-FUSION (ETV6-NTRK3 fusion). ESCAT IA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ntrk-fusion-crc",
        file="bma_bma_ntrk_fusion_crc.json",
        label_ua="DIS-CRC · BIO-NTRK-FUSION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-NTRK-FUSION (NTRK1/3 fusion (rare ~0.1-0.3%, enriched in MSI-H / dMMR CRC). ESCAT IA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-NTRK-FUSION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-NTRK-FUSION (NTRK1/3 fusion (rare ~0.1-0.3%, enriched in MSI-H / dMMR CRC). ESCAT IA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ntrk-fusion-gist",
        file="bma_bma_ntrk_fusion_gist.json",
        label_ua="DIS-GIST · BIO-NTRK-FUSION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-NTRK-FUSION (NTRK1/3 fusion (rare in KIT/PDGFRA wild-type GIST)). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GIST · BIO-NTRK-FUSION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-NTRK-FUSION (NTRK1/3 fusion (rare in KIT/PDGFRA wild-type GIST)). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ntrk-fusion-nsclc",
        file="bma_bma_ntrk_fusion_nsclc.json",
        label_ua="DIS-NSCLC · BIO-NTRK-FUSION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-NTRK-FUSION (NTRK1/3 fusion (rare ~0.1-0.5%)). ESCAT IA. Engine: 10 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-NSCLC · BIO-NTRK-FUSION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-NTRK-FUSION (NTRK1/3 fusion (rare ~0.1-0.5%)). ESCAT IA. Engine: 10 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ntrk-fusion-thyroid-papillary",
        file="bma_bma_ntrk_fusion_thyroid_papillary.json",
        label_ua="DIS-THYROID-PAPILLARY · BIO-NTRK-FUSION (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-NTRK-FUSION (NTRK1/3 fusion (rare in PTC, ~1-2%; enriched in radiation-in). ESCAT IA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-THYROID-PAPILLARY · BIO-NTRK-FUSION (ESCAT IA)",
        summary_en="Synthetic profile: BIO-NTRK-FUSION (NTRK1/3 fusion (rare in PTC, ~1-2%; enriched in radiation-in). ESCAT IA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-pdgfra-d842v-gist",
        file="bma_bma_pdgfra_d842v_gist.json",
        label_ua="DIS-GIST · BIO-PDGFRA (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-PDGFRA (D842V (activation loop, exon 18 — ~7% of GIST, predominantly). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GIST · BIO-PDGFRA (ESCAT IA)",
        summary_en="Synthetic profile: BIO-PDGFRA (D842V (activation loop, exon 18 — ~7% of GIST, predominantly). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-pdl1-cps-cervical",
        file="bma_bma_pdl1_cps_cervical.json",
        label_ua="DIS-CERVICAL · BIO-PDL1-CPS (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-PDL1-CPS (). ESCAT IA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CERVICAL · BIO-PDL1-CPS (ESCAT IA)",
        summary_en="Synthetic profile: BIO-PDL1-CPS (). ESCAT IA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-pik3r1-endometrial",
        file="bma_bma_pik3r1_endometrial.json",
        label_ua="DIS-ENDOMETRIAL · BIO-PIK3R1 (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-PIK3R1 (PIK3R1 activating/loss-of-function mutation — endometrial ca). ESCAT IIB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-ENDOMETRIAL · BIO-PIK3R1 (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-PIK3R1 (PIK3R1 activating/loss-of-function mutation — endometrial ca). ESCAT IIB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-pold1-germline-polymerase-proofreading",
        file="bma_bma_pold1_germline_polymerase_proofreading.json",
        label_ua="DIS-CRC · BIO-POLD1-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-POLD1-GERMLINE (POLD1 germline exonuclease-domain pathogenic (PPAP — polymer). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-POLD1-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-POLD1-GERMLINE (POLD1 germline exonuclease-domain pathogenic (PPAP — polymer). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-pole-germline-polymerase-proofreading",
        file="bma_bma_pole_germline_polymerase_proofreading.json",
        label_ua="DIS-CRC · BIO-POLE-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-POLE-GERMLINE (POLE germline exonuclease-domain pathogenic (PPAP — polymera). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-CRC · BIO-POLE-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-POLE-GERMLINE (POLE germline exonuclease-domain pathogenic (PPAP — polymera). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ptch1-bcc",
        file="bma_bma_ptch1_bcc.json",
        label_ua="DIS-BCC · BIO-PTCH1 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-PTCH1 (PTCH1 loss-of-function (somatic — sporadic BCC; germline — G). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-BCC · BIO-PTCH1 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-PTCH1 (PTCH1 loss-of-function (somatic — sporadic BCC; germline — G). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ptch1-germline-nbccs-gorlin",
        file="bma_bma_ptch1_germline_nbccs_gorlin.json",
        label_ua="DIS-BCC · BIO-PTCH1-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-PTCH1-GERMLINE (PTCH1 germline pathogenic (NBCCS / Gorlin syndrome)). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-BCC · BIO-PTCH1-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-PTCH1-GERMLINE (PTCH1 germline pathogenic (NBCCS / Gorlin syndrome)). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-ret-fusion-thyroid-papillary",
        file="bma_bma_ret_fusion_thyroid_papillary.json",
        label_ua="DIS-THYROID-PAPILLARY · BIO-RET (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-RET (fusion (CCDC6-RET, NCOA4-RET, ELKS-RET et al.) — distinct fr). ESCAT IB. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-THYROID-PAPILLARY · BIO-RET (ESCAT IB)",
        summary_en="Synthetic profile: BIO-RET (fusion (CCDC6-RET, NCOA4-RET, ELKS-RET et al.) — distinct fr). ESCAT IB. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-smarcb1-epithelioid-sarcoma",
        file="bma_bma_smarcb1_epithelioid_sarcoma.json",
        label_ua="DIS-EPITHELIOID-SARCOMA · BIO-SMARCB1 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-SMARCB1 (SMARCB1/INI1 biallelic loss-of-function — confirmed by loss ). ESCAT IA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-EPITHELIOID-SARCOMA · BIO-SMARCB1 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-SMARCB1 (SMARCB1/INI1 biallelic loss-of-function — confirmed by loss ). ESCAT IA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-smo-bcc",
        file="bma_bma_smo_bcc.json",
        label_ua="DIS-BCC · BIO-SMO (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-SMO (SMO activating mutation — activates HH pathway downstream of). ESCAT IA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-BCC · BIO-SMO (ESCAT IA)",
        summary_en="Synthetic profile: BIO-SMO (SMO activating mutation — activates HH pathway downstream of). ESCAT IA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-sufu-germline-medulloblastoma",
        file="bma_bma_sufu_germline_medulloblastoma.json",
        label_ua="DIS-GBM · BIO-SUFU-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-SUFU-GERMLINE (SUFU germline pathogenic (NBCCS-like, medulloblastoma-predom). ESCAT IIA. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GBM · BIO-SUFU-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-SUFU-GERMLINE (SUFU germline pathogenic (NBCCS-like, medulloblastoma-predom). ESCAT IIA. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tert-glioma",
        file="bma_bma_tert_glioma.json",
        label_ua="DIS-GBM · BIO-TERT (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-TERT (TERT promoter mutation (C228T or C250T) — WHO CNS tumor clas). ESCAT IIB. Engine: 3 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-GBM · BIO-TERT (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-TERT (TERT promoter mutation (C228T or C250T) — WHO CNS tumor clas). ESCAT IIB. Engine: 3 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tert-thyroid",
        file="bma_bma_tert_thyroid.json",
        label_ua="DIS-THYROID-PAPILLARY · BIO-TERT (ESCAT IIB)",
        summary_ua="Синтетичний профіль: BIO-TERT (TERT promoter mutation (C228T or C250T) in papillary or foll). ESCAT IIB. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-THYROID-PAPILLARY · BIO-TERT (ESCAT IIB)",
        summary_en="Synthetic profile: BIO-TERT (TERT promoter mutation (C228T or C250T) in papillary or foll). ESCAT IIB. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tp53-ihc-endometrial",
        file="bma_bma_tp53_ihc_endometrial.json",
        label_ua="DIS-ENDOMETRIAL · BIO-TP53-IHC (ESCAT IB)",
        summary_ua="Синтетичний профіль: BIO-TP53-IHC (p53-aberrant IHC pattern: either (a) diffuse strong nuclear ). ESCAT IB. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-ENDOMETRIAL · BIO-TP53-IHC (ESCAT IB)",
        summary_en="Synthetic profile: BIO-TP53-IHC (p53-aberrant IHC pattern: either (a) diffuse strong nuclear ). ESCAT IB. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tp53-mut-mcl",
        file="bma_bma_tp53_mut_mcl.json",
        label_ua="DIS-MCL · BIO-TP53-MUTATION (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-TP53-MUTATION (any pathogenic mutation OR del(17p)). ESCAT IIA. Engine: 2 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="b_aggressive",
        label_en="DIS-MCL · BIO-TP53-MUTATION (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-TP53-MUTATION (any pathogenic mutation OR del(17p)). ESCAT IIA. Engine: 2 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tp53-mut-mm",
        file="bma_bma_tp53_mut_mm.json",
        label_ua="DIS-MM · BIO-TP53-MUTATION (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-TP53-MUTATION (any pathogenic mutation OR del(17p)). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="myeloma",
        label_en="DIS-MM · BIO-TP53-MUTATION (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-TP53-MUTATION (any pathogenic mutation OR del(17p)). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tsc1-germline-tuberous-sclerosis",
        file="bma_bma_tsc1_germline_tuberous_sclerosis.json",
        label_ua="DIS-RCC · BIO-TSC1-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-TSC1-GERMLINE (TSC1 germline pathogenic (hamartin loss-of-function)). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-RCC · BIO-TSC1-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-TSC1-GERMLINE (TSC1 germline pathogenic (hamartin loss-of-function)). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tsc1-lam",
        file="bma_bma_tsc1_lam.json",
        label_ua="DIS-LAM · BIO-TSC1 (ESCAT IA)",
        summary_ua="Синтетичний профіль: BIO-TSC1 (TSC1 or TSC2 loss-of-function (LAM context — somatic in spor). ESCAT IA. Engine: 1 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-LAM · BIO-TSC1 (ESCAT IA)",
        summary_en="Synthetic profile: BIO-TSC1 (TSC1 or TSC2 loss-of-function (LAM context — somatic in spor). ESCAT IA. Engine: 1 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    CaseEntry(
        case_id="bma-bma-tsc2-germline-tuberous-sclerosis",
        file="bma_bma_tsc2_germline_tuberous_sclerosis.json",
        label_ua="DIS-RCC · BIO-TSC2-GERMLINE (ESCAT IIA)",
        summary_ua="Синтетичний профіль: BIO-TSC2-GERMLINE (TSC2 germline pathogenic (tuberin loss-of-function)). ESCAT IIA. Engine: 4 tracks. Не для клінічних рішень.",
        badge="Molecular example", badge_class="bdg-plan", category="solid",
        label_en="DIS-RCC · BIO-TSC2-GERMLINE (ESCAT IIA)",
        summary_en="Synthetic profile: BIO-TSC2-GERMLINE (TSC2 germline pathogenic (tuberin loss-of-function)). ESCAT IIA. Engine: 4 tracks. Not for clinical decisions.",
        quality_tier="molecular", scenario_type="molecular",
    ),
    # ── /AUTO-BMA-EXAMPLES ──
    ...
    # Wave L additions (2026-05-20): 44 unregistered example JSONs
    # promoted into prevention carousel (universal/iatrogenic/
    # occupational/environmental/autoimmune/hereditary phenotype pilots).
    CaseEntry(
        case_id='prevention-atrophic-gastritis',
        file='patient_atrophic_gastritis_chronic.json',
        label_ua='Chronic atrophic gastritis OLGIM III · EGD q3y per Sydney protocol',
        summary_ua='60-річна жінка з histologically confirmed chronic atrophic gastritis, OLGIM III, autoimmune (anti-parietal-cell + B12 deficiency, elevated gastrin). ~5-10× RR gastric AC + type 1 NET. Engine routes to PreventionPlan: стандарт = IND-CHRONIC-ATROPHIC-GASTRITIS-PREVENTION-SURVEILLANCE (EGD q3y Sydney biopsies + B12/iron); альтернатива = OBSERVATION (symptom-driven).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Chronic atrophic gastritis OLGIM III · EGD q3y per Sydney protocol',
        summary_en='60yo female with histology-confirmed chronic atrophic gastritis, OLGIM III, autoimmune (anti-parietal-cell + B12 deficiency, elevated gastrin). ~5-10× gastric AC RR + type 1 NET. Engine routes to PreventionPlan: standard = IND-CHRONIC-ATROPHIC-GASTRITIS-PREVENTION-SURVEILLANCE (EGD q3y Sydney biopsies + B12/iron); alternative = OBSERVATION (symptom-driven).',
    ),
    CaseEntry(
        case_id='prevention-autoimmune-hashimoto',
        file='patient_hashimoto_thyroiditis_lymphoma_risk.json',
        label_ua="Hashimoto's thyroiditis · thyroid MALT/DLBCL 67-80× risk · TSH + US q1y",
        summary_ua="48-річна жінка, confirmed Hashimoto's (anti-TPO + anti-Tg+, diffuse goiter, hypothyroidism on levothyroxine ×8р). ~67-80× thyroid MALT/DLBCL risk (Holm NEJM 1985 / Pedersen Histopath 1996). Engine спрацьовує RF-AUTOIMMUNE-THYROIDITIS-LYMPHOMA-PREVENTION → PreventionPlan: стандарт = TSH q6-12mo + thyroid US q1y + FNA на dominant/TIRADS-3+ nodule; альтернатива = OBSERVATION.",
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en="Hashimoto's thyroiditis · thyroid MALT/DLBCL 67-80× risk · TSH + US q1y",
        summary_en="48yo female, confirmed Hashimoto's (anti-TPO + anti-Tg+, diffuse goiter, hypothyroidism on levothyroxine ×8y). ~67-80× thyroid MALT/DLBCL risk (Holm NEJM 1985 / Pedersen Histopath 1996). Engine fires RF-AUTOIMMUNE-THYROIDITIS-LYMPHOMA-PREVENTION → PreventionPlan: standard = TSH q6-12mo + thyroid US q1y + FNA on dominant/TIRADS-3+ nodule; alternative = OBSERVATION.",
    ),
    CaseEntry(
        case_id='prevention-autoimmune-scleroderma',
        file='patient_scleroderma_lung_risk.json',
        label_ua='Systemic sclerosis + ILD (anti-Scl70+) · ~5-7× lung SIR · HRCT q1-2y',
        summary_ua='55-річна жінка з limited cutaneous SSc (ACR/EULAR 2013 criteria, anti-Scl70+) + established ILD (HRCT NSIP pattern). SSc-ILD subset ~5-7× lung cancer SIR (anti-Scl70 further elevates). Engine routes to PreventionPlan: стандарт = IND-SCLERODERMA-LUNG-PREVENTION-SURVEILLANCE (HRCT q1-2y + PFTs q6-12mo + smoking-cessation reinforcement); альтернатива = OBSERVATION (rheumatology-driven imaging on trigger).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Systemic sclerosis + ILD (anti-Scl70+) · ~5-7× lung SIR · HRCT q1-2y',
        summary_en='55yo female with limited cutaneous SSc (ACR/EULAR 2013 criteria, anti-Scl70+) + established ILD (HRCT NSIP pattern). SSc-ILD subset ~5-7× lung cancer SIR (anti-Scl70 further elevates). Engine routes to PreventionPlan: standard = IND-SCLERODERMA-LUNG-PREVENTION-SURVEILLANCE (HRCT q1-2y + PFTs q6-12mo + smoking-cessation reinforcement); alternative = OBSERVATION (rheumatology-driven imaging on trigger).',
    ),
    CaseEntry(
        case_id='prevention-autoimmune-sjogren',
        file='patient_sjogren_malt_risk.json',
        label_ua="Primary Sjögren's · ~15-20× MALT lymphoma RR · annual labs + parotid US",
        summary_ua="58-річна жінка з primary Sjögren's (EULAR/ACR 2016 criteria, anti-Ro/SSA + anti-La/SSB+, focal lymphocytic sialadenitis на minor salivary gland biopsy, sicca ×8р). Найвищий гематологічний-cancer ризик серед rheumatic — ~15-20× MALT lymphoma RR, lifetime ~5-10%. Engine routes to PreventionPlan: стандарт = annual exam + cryoglobulins/C4/SPEP/Ig + parotid US на нове/persistent swelling; альтернатива = OBSERVATION (rheumatology-driven).",
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en="Primary Sjögren's · ~15-20× MALT lymphoma RR · annual labs + parotid US",
        summary_en="58yo female with primary Sjögren's (EULAR/ACR 2016, anti-Ro/SSA + anti-La/SSB+, focal lymphocytic sialadenitis on minor salivary gland biopsy, sicca ×8y). Highest hematologic-cancer risk among rheumatic — ~15-20× MALT lymphoma RR, lifetime ~5-10%. Engine routes to PreventionPlan: standard = annual exam + cryoglobulins/C4/SPEP/Ig + parotid US on new/persistent swelling; alternative = OBSERVATION (rheumatology-driven).",
    ),
    CaseEntry(
        case_id='prevention-bap1-uveal-pedigree',
        file='patient_bap1_pedigree_uveal_melanoma.json',
        label_ua='BAP1 suspicion · father uveal melanoma + mother mesothelioma',
        summary_ua='33-річна жінка, asymptomatic. Father uveal melanoma 52 + mother pleural mesothelioma 60 (двоє first-degree, BAP1-spectrum кластер). Germline BAP1 ще не зроблений. Engine спрацьовує RF-BAP1-FAMILY-HISTORY-SUSPICION → PreventionPlan: counseling + germline BAP1 panel; cascade на carrier RFs якщо patho-variant (annual ophth/skin/abdominal MRI).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='BAP1 suspicion · father uveal melanoma + mother mesothelioma',
        summary_en='33yo female, asymptomatic. Father uveal melanoma 52 + mother pleural mesothelioma 60 (two first-degree BAP1-spectrum cluster). Germline BAP1 not yet performed. Engine fires RF-BAP1-FAMILY-HISTORY-SUSPICION → PreventionPlan: counseling + germline BAP1 panel; cascade to carrier RFs if pathogenic (annual ophth/skin/abdominal MRI).',
    ),
    CaseEntry(
        case_id='prevention-brca2-male-carrier',
        file='patient_brca2_male_breast_carrier.json',
        label_ua='BRCA2 male carrier (c.5946delT founder) · male breast + prostate + PDAC',
        summary_ua="47-річний чоловік, confirmed germline BRCA2 c.5946delT (cascade після sister's BRCA2 ovarian cancer 51). Engine спрацьовує RF-BRCA-CONFIRMED-CARRIER → PreventionPlan: стандарт = male breast CBE q6-12mo з 35, mammography 50, PSA + DRE з 40, pancreatic surveillance discussion; aggressive = annual mpMRI prostate з 40 + EUS/MRCP з 50.",
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='BRCA2 male carrier (c.5946delT founder) · male breast + prostate + PDAC',
        summary_en="47yo male, confirmed germline BRCA2 c.5946delT (cascade after sister's BRCA2 ovarian cancer at 51). Engine fires RF-BRCA-CONFIRMED-CARRIER → PreventionPlan: standard = male breast CBE q6-12mo from 35, mammography 50, PSA + DRE from 40, pancreatic surveillance discussion; aggressive = annual mpMRI prostate from 40 + EUS/MRCP from 50.",
    ),
    CaseEntry(
        case_id='prevention-cowden-phenotype',
        file='patient_cowden_macrocephaly_phenotype.json',
        label_ua='Cowden phenotype · macrocephaly + trichilemmomas + multinodular goiter',
        summary_ua='38-річна жінка з macrocephaly (OFC 59 cm) + multiple trichilemmomas + acral keratoses + multinodular goiter — meets Cleveland Clinic/NCCN Cowden clinical criteria. Engine спрацьовує RF-COWDEN-FAMILY-HISTORY-SUSPICION (clinical-criteria branch) → PreventionPlan: стандарт = pre-test counseling + germline PTEN panel; альтернатива = empirical multi-organ Cowden-style surveillance.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Cowden phenotype · macrocephaly + trichilemmomas + multinodular goiter',
        summary_en='38yo female with macrocephaly (OFC 59 cm) + multiple trichilemmomas + acral keratoses + multinodular goiter — meets Cleveland Clinic/NCCN Cowden clinical criteria. Engine fires RF-COWDEN-FAMILY-HISTORY-SUSPICION (clinical-criteria branch) → PreventionPlan: standard = pre-test counseling + germline PTEN panel; alternative = empirical multi-organ Cowden-style surveillance.',
    ),
    CaseEntry(
        case_id='prevention-env-arsenic-water',
        file='patient_arsenic_water_contamination.json',
        label_ua='Drinking-water arsenic >50 µg/L · urothelial + skin + lung surveillance',
        summary_ua='50-річний чоловік, lifelong rural resident, private-well inorganic arsenic >50 µg/L (>WHO/EPA 10 µg/L з 2006р). IARC Group 1. Engine спрацьовує RF-ENVIRONMENTAL-ARSENIC-WATER-PREVENTION → PreventionPlan: стандарт = remediation (POU filtration), urinalysis + cytology q1-2y bladder surveillance, annual dermatologic exam; aggressive = enhanced cadence + lung imaging.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Drinking-water arsenic >50 µg/L · urothelial + skin + lung surveillance',
        summary_en='50yo male, lifelong rural resident, private-well inorganic arsenic >50 µg/L (>WHO/EPA 10 µg/L since 2006). IARC Group 1. Engine fires RF-ENVIRONMENTAL-ARSENIC-WATER-PREVENTION → PreventionPlan: standard = remediation (POU filtration), urinalysis + cytology q1-2y for bladder, annual dermatologic exam; aggressive = enhanced cadence + lung imaging.',
    ),
    CaseEntry(
        case_id='prevention-env-outdoor-uv',
        file='patient_outdoor_uv_construction_worker.json',
        label_ua='Outdoor UV occupational 25y (construction) · BCC/SCC/melanoma',
        summary_ua='50-річний чоловік, outdoor construction worker ×25р у temperate-to-southern UA latitudes (intermittent equatorial), Fitzpatrick II, multiple sunburns. Engine routes to PreventionPlan: photoprotection counseling (broad-spectrum sunscreen + clothing + hat) + annual clinical skin exam + self-exam education + vitamin D balance. Validates v0.3 RF-ENVIRONMENTAL-OUTDOOR-UV-SKIN-PREVENTION pilot.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Outdoor UV occupational 25y (construction) · BCC/SCC/melanoma',
        summary_en='50yo male outdoor construction worker ×25y in temperate-to-southern UA latitudes (intermittent equatorial), Fitzpatrick II, multiple sunburns. Engine routes to PreventionPlan: photoprotection counseling (broad-spectrum sunscreen + clothing + hat) + annual clinical skin exam + self-exam education + vitamin D balance. Validates v0.3 RF-ENVIRONMENTAL-OUTDOOR-UV-SKIN-PREVENTION pilot.',
    ),
    CaseEntry(
        case_id='prevention-env-pm25',
        file='patient_pm25_high_pollution_area.json',
        label_ua='PM2.5 ~48 µg/m³ (high-pollution urban) · non-smoker lung surveillance',
        summary_ua='42-річна жінка non-smoker, lifelong resident високо-PM2.5 industrial-adjacent urban district (annual PM2.5 ~48 µg/m³, >WHO IT-1 35 µg/m³). IARC Group 1 (Monograph 109, 2013) — кожен +10 µg/m³ ↑lung ~9%. Engine спрацьовує RF-ENVIRONMENTAL-AIR-POLLUTION-LUNG-PREVENTION → PreventionPlan: стандарт = exposure-mitigation (indoor HEPA + N95 high-AQI + advocacy/relocation SDM), USPSTF lung screening eligibility review; aggressive = enhanced surveillance.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='PM2.5 ~48 µg/m³ (high-pollution urban) · non-smoker lung surveillance',
        summary_en='42yo female non-smoker, lifelong high-PM2.5 industrial-adjacent urban district (annual PM2.5 ~48 µg/m³, >WHO IT-1 35 µg/m³). IARC Group 1 (Monograph 109, 2013) — each +10 µg/m³ ↑lung ~9%. Engine fires RF-ENVIRONMENTAL-AIR-POLLUTION-LUNG-PREVENTION → PreventionPlan: standard = exposure-mitigation (indoor HEPA + N95 high-AQI + advocacy/relocation SDM), USPSTF lung screening eligibility review; aggressive = enhanced surveillance.',
    ),
    CaseEntry(
        case_id='prevention-env-secondhand-smoke',
        file='patient_secondhand_smoke_household.json',
        label_ua='Lifelong household ETS (parents + spouse) · ~20-25% ↑lung RR',
        summary_ua='45-річна жінка non-smoker з sustained household ETS exposure — childhood (обидва батьки курці indoors 1980-90s) + adult cohabitation з active-smoker spouse ×20р. IARC Group 1 + US Surgeon General. Engine routes to PreventionPlan: cohabitant cessation support + workplace smoke-free environment + USPSTF screening eligibility check (likely ineligible) + symptom-triggered evaluation.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Lifelong household ETS (parents + spouse) · ~20-25% ↑lung RR',
        summary_en='45yo female non-smoker with sustained household ETS exposure — childhood (both parents smoking indoors 1980s-90s) + adult cohabitation with active-smoker spouse ×20y. IARC Group 1 + US Surgeon General. Engine routes to PreventionPlan: cohabitant cessation support + workplace smoke-free environment + USPSTF screening eligibility check (likely ineligible) + symptom-triggered evaluation.',
    ),
    CaseEntry(
        case_id='prevention-fammm-pedigree',
        file='patient_familial_melanoma_cdkn2a_pedigree.json',
        label_ua='FAMMM suspicion (CDKN2A) · melanoma 32 + paternal PDAC 55',
        summary_ua='28-річний чоловік: father cutaneous melanoma 32 (Clark IV, BRAF V600E) + paternal grandfather PDAC 55; personal ~30 atypical nevi (дисплазний фенотип). Engine спрацьовує RF-FAMMM-FAMILY-HISTORY-SUSPICION (melanoma + pancreatic cluster) → PreventionPlan: стандарт = pre-test counseling + germline CDKN2A panel; альтернатива = empirical FAMMM surveillance (annual full-body skin + TBP + EUS/MRI pancreatic з 40 per CAPS).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='FAMMM suspicion (CDKN2A) · melanoma 32 + paternal PDAC 55',
        summary_en='28yo male: father cutaneous melanoma 32 (Clark IV, BRAF V600E) + paternal grandfather PDAC 55; personal ~30 atypical nevi (dysplastic phenotype). Engine fires RF-FAMMM-FAMILY-HISTORY-SUSPICION (melanoma + pancreatic cluster) → PreventionPlan: standard = pre-test counseling + germline CDKN2A panel; alternative = empirical FAMMM surveillance (annual full-body skin + TBP + EUS/MRI pancreatic from 40 per CAPS).',
    ),
    CaseEntry(
        case_id='prevention-iatrogenic-combined-hrt',
        file='patient_combined_hrt_5y_breast_risk.json',
        label_ua='Combined HRT ≥5y · breast/VTE risk-benefit + taper-discontinue option',
        summary_ua='58-річна жінка, continuous-combined HRT (E2 1mg + MPA 2.5mg) ×6р після surgical menopause, intact uterus. IARC Group 1 (≥5y тригер). Engine спрацьовує RF-IATROGENIC-COMBINED-HRT-PREVENTION → PreventionPlan: стандарт = shared-decision risk-benefit + lowest-dose/shortest-duration per NAMS/ACOG, USPSTF breast cadence; альтернатива = guideline taper-and-discontinue + non-hormonal vasomotor management.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Combined HRT ≥5y · breast/VTE risk-benefit + taper-discontinue option',
        summary_en='58yo female, continuous-combined HRT (E2 1mg + MPA 2.5mg) ×6y after surgical menopause, intact uterus. IARC Group 1 (≥5y trigger). Engine fires RF-IATROGENIC-COMBINED-HRT-PREVENTION → PreventionPlan: standard = shared-decision risk-benefit + lowest-dose/shortest-duration per NAMS/ACOG, USPSTF breast cadence; alternative = guideline taper-and-discontinue + non-hormonal vasomotor management.',
    ),
    CaseEntry(
        case_id='prevention-iatrogenic-des',
        file='patient_des_exposure_in_utero.json',
        label_ua='DES daughter (in-utero exposure) · vaginal CCA + CIN + breast surveillance',
        summary_ua="60-річна жінка ('DES daughter', born 1965, maternal DES 1964-65). Cumulative CCA risk ~1/1000, bimodal age peaks (15-25 + post-40). Cervical CIN + modestly elevated breast risk post-40. Engine спрацьовує RF-IATROGENIC-DES-EXPOSURE-PREVENTION → PreventionPlan: annual DES-specific gyn exam (speculum + bimanual + careful vaginal palpation per ACOG), colposcopy + iodine staining, cytology + targeted breast surveillance.",
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='DES daughter (in-utero exposure) · vaginal CCA + CIN + breast surveillance',
        summary_en="60yo female ('DES daughter', born 1965, maternal DES 1964-65). Cumulative CCA risk ~1/1000, bimodal age peaks (15-25 + post-40). Cervical CIN + modestly elevated breast risk post-40. Engine fires RF-IATROGENIC-DES-EXPOSURE-PREVENTION → PreventionPlan: annual DES-specific gyn exam (speculum + bimanual + careful vaginal palpation per ACOG), colposcopy + iodine staining, cytology + targeted breast surveillance.",
    ),
    CaseEntry(
        case_id='prevention-iatrogenic-ppi',
        file='patient_ppi_long_term_user.json',
        label_ua='Long-term PPI ≥10y · gastric NET surveillance + de-prescription',
        summary_ua='66-річний чоловік на omeprazole 20mg daily ×12р continuous для chronic GERD, no upper-GI symptoms, no prior EGD. Engine спрацьовує RF-IATROGENIC-LONG-TERM-PPI-GASTRIC-NET-PREVENTION → PreventionPlan: стандарт = indication-review + de-prescription (step-down to H2 / on-demand), fasting gastrin + CgA baseline, EGD body/fundus biopsies якщо persistent + EGD q3-5y якщо atrophic gastritis / ECL hyperplasia; aggressive = baseline EGD with biopsy mapping + EUS на ECL nodule >5mm + endocrinology referral якщо hypergastrinemia.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Long-term PPI ≥10y · gastric NET surveillance + de-prescription',
        summary_en='66yo male on omeprazole 20mg daily ×12y continuous for chronic GERD, no upper-GI symptoms, no prior EGD. Engine fires RF-IATROGENIC-LONG-TERM-PPI-GASTRIC-NET-PREVENTION → PreventionPlan: standard = indication-review + de-prescription (step-down to H2 / on-demand), fasting gastrin + CgA baseline, EGD body/fundus biopsies if persistent + EGD q3-5y if atrophic gastritis / ECL hyperplasia; aggressive = baseline EGD with biopsy mapping + EUS on ECL nodule >5mm + endocrinology referral if hypergastrinemia.',
    ),
    CaseEntry(
        case_id='prevention-lifestyle-low-fiber',
        file='patient_low_fiber_diet_crc_risk.json',
        label_ua='Low-fibre Western diet (~10 g/day) · CRC risk · counseling + screening',
        summary_ua='45-річний чоловік, habitual Western diet ~10 g fibre/day (значно нижче WCRF/AICR ≥30 g threshold). WCRF CUP: convincing-protective; кожні 10 g/day ↓CRC ~10%. Engine спрацьовує RF-LIFESTYLE-LOW-FIBER-CRC-PREVENTION → PreventionPlan: dietary counseling (whole-grain / veg / fruit / pulse upshift до ≥30 g/day) + USPSTF CRC screening (colonoscopy 45р baseline + interval per findings).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Low-fibre Western diet (~10 g/day) · CRC risk · counseling + screening',
        summary_en='45yo male, habitual Western diet ~10 g fibre/day (well below WCRF/AICR ≥30 g threshold). WCRF CUP rates convincing-protective; each 10 g/day ↓CRC ~10%. Engine fires RF-LIFESTYLE-LOW-FIBER-CRC-PREVENTION → PreventionPlan: dietary counseling (whole-grain / veg / fruit / pulse upshift to ≥30 g/day target) + USPSTF CRC screening (colonoscopy 45y baseline + interval per findings).',
    ),
    CaseEntry(
        case_id='prevention-lifestyle-red-meat',
        file='patient_processed_meat_high_intake.json',
        label_ua='High red + processed meat · CRC risk · counseling + USPSTF screening',
        summary_ua='52-річний чоловік, ~600 g/тиждень unprocessed red meat (>WCRF 500 g/тиждень) AND ~75 g/day processed meat (>IARC 50 g/day). IARC Monograph 114 (2015): processed meat Group 1 (CRC sufficient); red meat Group 2A. Кожні 50 g/day processed ↑CRC ~18%; 100 g/day red ↑CRC ~17%. Engine спрацьовує RF-LIFESTYLE-HIGH-RED-MEAT-CRC-PREVENTION → PreventionPlan: dietary counseling (red <500 g/тиждень + minimize processed) + USPSTF CRC (colonoscopy 45р baseline).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='High red + processed meat · CRC risk · counseling + USPSTF screening',
        summary_en='52yo male, ~600 g/week unprocessed red meat (>WCRF 500 g/week) AND ~75 g/day processed meat (>IARC 50 g/day). IARC Monograph 114 (2015): processed meat Group 1 (CRC sufficient); red meat Group 2A. Each 50 g/day processed ↑CRC ~18%; 100 g/day red ↑CRC ~17%. Engine fires RF-LIFESTYLE-HIGH-RED-MEAT-CRC-PREVENTION → PreventionPlan: dietary counseling (red <500 g/week + minimize processed) + USPSTF CRC (colonoscopy 45y baseline).',
    ),
    CaseEntry(
        case_id='prevention-men1-carrier',
        file='patient_men1_men_carrier_confirmed.json',
        label_ua="MEN1 confirmed carrier · cascade after sister's insulinoma 28",
        summary_ua="32-річна жінка, confirmed germline MEN1 c.249_252delGTCT (cascade після sister's insulinoma 28). Engine спрацьовує RF-MEN1-CONFIRMED-CARRIER → PreventionPlan: стандарт = IND-MEN1-CARRIER-SURVEILLANCE (annual PTH/Ca/prolactin/IGF-1/CgA + pituitary MRI q3y + abdominal MRI q1y); aggressive = IND-MEN1-CARRIER-INTENSIFIED (+ Zollinger-Ellison surveillance + prophylactic-parathyroidectomy SDM).",
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en="MEN1 confirmed carrier · cascade after sister's insulinoma 28",
        summary_en="32yo female, confirmed germline MEN1 c.249_252delGTCT (cascade after sister's insulinoma 28). Engine fires RF-MEN1-CONFIRMED-CARRIER → PreventionPlan: standard = IND-MEN1-CARRIER-SURVEILLANCE (annual PTH/Ca/prolactin/IGF-1/CgA + pituitary MRI q3y + abdominal MRI q1y); aggressive = IND-MEN1-CARRIER-INTENSIFIED (+ Zollinger-Ellison surveillance + prophylactic-parathyroidectomy SDM).",
    ),
    CaseEntry(
        case_id='prevention-men2-carrier',
        file='patient_men2_prophylactic_thyroidectomy_pediatric.json',
        label_ua='MEN2B (RET M918T, HST class) · urgent prophylactic thyroidectomy at 5',
        summary_ua='5-річний хлопчик, confirmed germline RET M918T (MEN2B/HST class — highest-risk per ATA 2015). Carrier classification мандатує prophylactic thyroidectomy ideally in infancy; presenting at 5 — late/urgent surgical scenario. Engine спрацьовує RF-MEN2-CONFIRMED-CARRIER → PreventionPlan: стандарт = prophylactic total thyroidectomy + central neck dissection + calcitonin q6mo + metanephrines q1y від 8; aggressive = comprehensive workup + bilateral adrenalectomy planning якщо bilateral pheo emerges.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='MEN2B (RET M918T, HST class) · urgent prophylactic thyroidectomy at 5',
        summary_en='5yo boy, confirmed germline RET M918T (MEN2B/HST class — highest-risk per ATA 2015). Classification mandates prophylactic thyroidectomy ideally in infancy; presenting at 5 — late/urgent surgical scenario. Engine fires RF-MEN2-CONFIRMED-CARRIER → PreventionPlan: standard = prophylactic total thyroidectomy + central neck dissection + calcitonin q6mo + metanephrines q1y from 8; aggressive = comprehensive workup + bilateral adrenalectomy planning if bilateral pheo emerges.',
    ),
    CaseEntry(
        case_id='prevention-mpn-pv-secondary',
        file='patient_mpn_pv_secondary_aml_risk.json',
        label_ua='JAK2+ pre-PV phase · secondary-AML/MF surveillance',
        summary_ua='65-річний чоловік, JAK2 V617F+ (allele burden 32%) з PV-suggestive phenotype (Hct 53% + ↓EPO + mild splenomegaly), bone marrow pending, NOT yet meeting WHO 2022 PV criteria. Pre-MPN-criteria phase. Engine routes to PreventionPlan: стандарт = IND-MPN-SECONDARY-LEUKEMIA-PREVENTION-SURVEILLANCE (CBC + smear q3-6mo + BM на cytopenia/blast + JAK2 allele-burden monitoring); альтернатива = OBSERVATION (hematology-driven).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='JAK2+ pre-PV phase · secondary-AML/MF surveillance',
        summary_en='65yo male, JAK2 V617F+ (allele burden 32%) with PV-suggestive phenotype (Hct 53% + ↓EPO + mild splenomegaly), BM pending, NOT yet meeting WHO 2022 PV criteria. Pre-MPN-criteria phase. Engine routes to PreventionPlan: standard = IND-MPN-SECONDARY-LEUKEMIA-PREVENTION-SURVEILLANCE (CBC + smear q3-6mo + BM on cytopenia/blast + JAK2 allele-burden monitoring); alternative = OBSERVATION (hematology-driven).',
    ),
    CaseEntry(
        case_id='prevention-multi-exposure-smoker-radon',
        file='patient_multiple_exposures_smoker_radon.json',
        label_ua='Stacked RFs: current smoker + residential radon 7.2 pCi/L · synergy',
        summary_ua='65-річний active smoker (30 pack-y) + residential radon 7.2 pCi/L (>EPA 4 pCi/L action) у granite-bedrock region. Stacks два upstream RFs: (1) RF-UNIVERSAL-CURRENT-SMOKER-MULTI-CANCER → cessation + USPSTF LDCT; (2) RF-RADON-MALIGNANCY-PREVENTION → sub-slab depressurization + cessation amplification (radon-smoking synergy). Validates RF-stacking + cumulative lung-cancer risk display.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Stacked RFs: current smoker + residential radon 7.2 pCi/L · synergy',
        summary_en='65yo active smoker (30 pack-y) + residential radon 7.2 pCi/L (>EPA 4 pCi/L action) in granite-bedrock region. Stacks two upstream RFs: (1) RF-UNIVERSAL-CURRENT-SMOKER-MULTI-CANCER → cessation + USPSTF LDCT; (2) RF-RADON-MALIGNANCY-PREVENTION → sub-slab depressurization + cessation amplification (radon-smoking synergy). Validates RF-stacking + cumulative lung-cancer risk display.',
    ),
    CaseEntry(
        case_id='prevention-occ-chromium',
        file='patient_chromium_plating_worker.json',
        label_ua='Hexavalent chromium (Cr(VI)) electroplating · lung + sinonasal surveillance',
        summary_ua='55-річний чоловік, 25р chrome-plating-line operator (Cr(VI) inhalation + dermal). IARC Group 1 (Monograph 100C, 2012) — lung cancer + nasal/paranasal-sinus. Engine routing pending RF-OCC-HEXAVALENT-CHROMIUM-PREVENTION authoring (v0.3 Wave I); forward-compatible findings keys; profile carries narrative context until RF lands.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Hexavalent chromium (Cr(VI)) electroplating · lung + sinonasal surveillance',
        summary_en='55yo male, 25 years chrome-plating-line operator (Cr(VI) inhalation + dermal). IARC Group 1 (Monograph 100C, 2012) — lung + nasal/paranasal-sinus cancer. Engine routing pending RF-OCC-HEXAVALENT-CHROMIUM-PREVENTION authoring (v0.3 Wave I); forward-compatible findings keys; profile carries narrative context until RF lands.',
    ),
    CaseEntry(
        case_id='prevention-occ-diesel-exhaust',
        file='patient_diesel_exhaust_truck_driver.json',
        label_ua='Diesel-exhaust 25y (truck driver) · LDCT + smoking + bladder surveillance',
        summary_ua='58-річний чоловік, retired long-haul truck driver ×25р DEE exposure (1995-2020). IARC Group 1 (Monograph 105, 2012) — lung primary + bladder limited-suggestive. Engine спрацьовує RF-OCC-DIESEL-EXHAUST-PREVENTION → PreventionPlan: стандарт = risk-adjusted LDCT + smoking-cessation + urinalysis q1-2y; альтернатива = USPSTF general-population only.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Diesel-exhaust 25y (truck driver) · LDCT + smoking + bladder surveillance',
        summary_en='58yo male, retired long-haul truck driver ×25y DEE (1995-2020). IARC Group 1 (Monograph 105, 2012) — lung primary + bladder limited-suggestive. Engine fires RF-OCC-DIESEL-EXHAUST-PREVENTION → PreventionPlan: standard = risk-adjusted LDCT + smoking-cessation + urinalysis q1-2y bladder; alternative = USPSTF general-population only.',
    ),
    CaseEntry(
        case_id='prevention-occ-firefighter',
        file='patient_firefighter_career.json',
        label_ua='Firefighter career 22y · IARC Group 1 (2022) · LDCT + skin + bladder',
        summary_ua='48-річний чоловік, career municipal structural firefighter ×22р. IARC reclassified Group 1 (Monograph 132, 2022) — sufficient evidence bladder + mesothelioma; limited prostate/colon/melanoma/NHL/testicular. Engine спрацьовує RF-OCC-FIREFIGHTER-PREVENTION → PreventionPlan: стандарт = SCBA + decontamination + USPSTF-modified LDCT + derm exam + CRC від 45р + urinalysis q1-2y; альтернатива = USPSTF general only.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Firefighter career 22y · IARC Group 1 (2022) · LDCT + skin + bladder',
        summary_en='48yo male, career municipal structural firefighter ×22y. IARC reclassified Group 1 (Monograph 132, 2022) — sufficient evidence bladder + mesothelioma; limited prostate/colon/melanoma/NHL/testicular. Engine fires RF-OCC-FIREFIGHTER-PREVENTION → PreventionPlan: standard = SCBA + decontamination + USPSTF-modified LDCT + derm exam + CRC from 45 + urinalysis q1-2y; alternative = USPSTF general only.',
    ),
    CaseEntry(
        case_id='prevention-occ-formaldehyde',
        file='patient_formaldehyde_pathologist.json',
        label_ua='Formaldehyde 20y (pathologist) · nasopharyngeal + AML surveillance',
        summary_ua='50-річна жінка, anatomical pathologist ×20р formaldehyde (HCHO + formalin) exposure. IARC Group 1 (Monograph 100F, 2012) — NPC (latency 20-40y) + myeloid leukemia/AML (latency 5-15y). Engine спрацьовує RF-OCC-FORMALDEHYDE-PREVENTION → PreventionPlan: стандарт = PPE + routine CBC + ENT symptom-aware NP exam; aggressive = annual CBC + baseline NP scopy + symptom-prompted MRI.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Formaldehyde 20y (pathologist) · nasopharyngeal + AML surveillance',
        summary_en='50yo female, anatomical pathologist ×20y formaldehyde (HCHO + formalin). IARC Group 1 (Monograph 100F, 2012) — NPC (latency 20-40y) + myeloid leukemia/AML (latency 5-15y). Engine fires RF-OCC-FORMALDEHYDE-PREVENTION → PreventionPlan: standard = PPE + routine CBC + ENT symptom-aware NP exam; aggressive = annual CBC + baseline NP scopy + symptom-prompted MRI.',
    ),
    CaseEntry(
        case_id='prevention-occ-ionizing-radiation-tech',
        file='patient_ionizing_radiation_radiologist.json',
        label_ua='Radiologic technologist 22y · CBC q1y + thyroid US q3-5y',
        summary_ua='48-річна жінка radiologic technologist ×22р в IR-suite (fluoroscopy-heavy). Engine routes to PreventionPlan: стандарт = IND-OCC-IONIZING-RADIATION-PREVENTION-INTENSIFIED-SURVEILLANCE (CBC q1y + thyroid US q3-5y + lifetime-dose review + ALARA counseling + standard breast screening); альтернатива = OBSERVATION. Validates v0.3 RF-OCC-IONIZING-RADIATION-PREVENTION pilot (AML/thyroid/breast/lung).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Radiologic technologist 22y · CBC q1y + thyroid US q3-5y',
        summary_en='48yo female radiologic technologist ×22y in IR suite (fluoroscopy-heavy). Engine routes to PreventionPlan: standard = IND-OCC-IONIZING-RADIATION-PREVENTION-INTENSIFIED-SURVEILLANCE (CBC q1y + thyroid US q3-5y + lifetime-dose review + ALARA counseling + standard breast screening); alternative = OBSERVATION. Validates v0.3 RF-OCC-IONIZING-RADIATION-PREVENTION pilot (AML/thyroid/breast/lung).',
    ),
    CaseEntry(
        case_id='prevention-occ-nickel',
        file='patient_nickel_refining_worker.json',
        label_ua='Nickel-refining 28y · lung + sinonasal surveillance (RF pending)',
        summary_ua='58-річний чоловік, 28р nickel-refining-plant worker (sulfide-ore smelting + electrolytic refining; mixed Ni sulfide/oxide/soluble-Ni dust inhalation). IARC Group 1 (Monograph 100C, 2012). Engine routing pending RF-OCC-NICKEL-COMPOUNDS-PREVENTION authoring (v0.3 Wave I); forward-compatible findings keys; profile carries narrative context until RF lands.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Nickel-refining 28y · lung + sinonasal surveillance (RF pending)',
        summary_en='58yo male, 28y nickel-refining-plant worker (sulfide-ore smelting + electrolytic refining; mixed Ni sulfide/oxide/soluble-Ni dust inhalation). IARC Group 1 (Monograph 100C, 2012). Engine routing pending RF-OCC-NICKEL-COMPOUNDS-PREVENTION authoring (v0.3 Wave I); forward-compatible findings keys; profile carries narrative context until RF lands.',
    ),
    CaseEntry(
        case_id='prevention-occ-pah',
        file='patient_pah_coke_oven_worker.json',
        label_ua='PAH coke-oven 20y · lung + bladder + skin surveillance',
        summary_ua='56-річний чоловік, 20р coke-oven worker (integrated steelmaking, Soviet-era plant 1990-2010). PAH-rich exposure (IARC Group 1, Monograph 100F). Engine routes to PreventionPlan: стандарт = IND-OCC-PAH-PREVENTION-INTENSIFIED-SURVEILLANCE (LDCT + urinalysis q1-2y + annual derm + smoking-cessation); альтернатива = OBSERVATION.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='PAH coke-oven 20y · lung + bladder + skin surveillance',
        summary_en='56yo male, 20y coke-oven worker (integrated steelmaking, Soviet-era plant 1990-2010). PAH-rich exposure (IARC Group 1, Monograph 100F). Engine routes to PreventionPlan: standard = IND-OCC-PAH-PREVENTION-INTENSIFIED-SURVEILLANCE (LDCT + urinalysis q1-2y + annual derm + smoking-cessation); alternative = OBSERVATION.',
    ),
    CaseEntry(
        case_id='prevention-occ-painters',
        file='patient_painters_career_lung_risk.json',
        label_ua='Industrial painter 25y · lung + bladder + AML surveillance',
        summary_ua='60-річний чоловік, 25р як industrial painter (construction + ship-painting, pre-1990s solvent-heavy). Engine routes to PreventionPlan: стандарт = IND-OCC-PAINTERS-PREVENTION-INTENSIFIED-SURVEILLANCE (LDCT якщо smoker-eligible + urinalysis q1-2y + CBC q1y + smoking-cessation); альтернатива = OBSERVATION (USPSTF general-population only).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Industrial painter 25y · lung + bladder + AML surveillance',
        summary_en='60yo male, 25y industrial painter (construction + ship-painting, pre-1990s solvent-heavy). Engine routes to PreventionPlan: standard = IND-OCC-PAINTERS-PREVENTION-INTENSIFIED-SURVEILLANCE (LDCT if smoker-eligible + urinalysis q1-2y + CBC q1y + smoking-cessation); alternative = OBSERVATION (USPSTF general-population only).',
    ),
    CaseEntry(
        case_id='prevention-occ-radiologist',
        file='patient_radiologist_25y_occupational.json',
        label_ua='Radiologist 25y (diagnostic + IR) · CBC + thyroid US surveillance',
        summary_ua='50-річний чоловік diagnostic + interventional radiologist ×25р (mixed fluoroscopy + CT-guided procedures), lifetime cumulative dose per ALARA dosimetry. Engine спрацьовує RF-OCC-IONIZING-RADIATION-PREVENTION → PreventionPlan: стандарт = IND-OCC-IONIZING-RADIATION-PREVENTION-INTENSIFIED-SURVEILLANCE (CBC q1y + thyroid US q3-5y + lifetime-dose review + ALARA/PPE counseling); альтернатива = OBSERVATION. Distinct demographic vs prevention-occ-ionizing-radiation-tech.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Radiologist 25y (diagnostic + IR) · CBC + thyroid US surveillance',
        summary_en='50yo male diagnostic + interventional radiologist ×25y (mixed fluoroscopy + CT-guided procedures), lifetime cumulative dose per ALARA dosimetry. Engine fires RF-OCC-IONIZING-RADIATION-PREVENTION → PreventionPlan: standard = IND-OCC-IONIZING-RADIATION-PREVENTION-INTENSIFIED-SURVEILLANCE (CBC q1y + thyroid US q3-5y + lifetime-dose review + ALARA/PPE counseling); alternative = OBSERVATION. Distinct demographic vs prevention-occ-ionizing-radiation-tech.',
    ),
    CaseEntry(
        case_id='prevention-occ-silica',
        file='patient_silica_stonemason.json',
        label_ua='RCS silica + ILO 1/1 silicosis · LDCT + smoking-cessation',
        summary_ua='55-річний чоловік, stonemason / engineered-quartz fabricator ×25р RCS exposure + established radiographic silicosis (HRCT, ILO 1/1). IARC Group 1 (Monograph 100C, 2012). Engine спрацьовує RF-OCC-SILICA-PREVENTION → PreventionPlan: стандарт = smoking-cessation + risk-adjusted LDCT + pulmonology co-management; aggressive = annual LDCT + sputum cytology SDM + earlier-cohort entry.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='RCS silica + ILO 1/1 silicosis · LDCT + smoking-cessation',
        summary_en='55yo male, stonemason / engineered-quartz fabricator ×25y RCS exposure + established radiographic silicosis (HRCT, ILO 1/1). IARC Group 1 (Monograph 100C, 2012). Engine fires RF-OCC-SILICA-PREVENTION → PreventionPlan: standard = smoking-cessation + risk-adjusted LDCT + pulmonology co-management; aggressive = annual LDCT + sputum cytology SDM + earlier-cohort entry.',
    ),
    CaseEntry(
        case_id='prevention-occ-wood-dust',
        file='patient_wood_dust_cabinetmaker.json',
        label_ua='Hardwood dust 25y (cabinetmaker) · sinonasal adenocarcinoma surveillance',
        summary_ua='52-річний cabinetmaker, 25р bespoke furniture workshop (oak/walnut/mahogany hardwood dust, pre-2010s dust-extraction era). IARC Group 1 (Monograph 100C); sinonasal AC latency 30-40р. Engine routes to PreventionPlan: стандарт = IND-OCC-WOOD-LEATHER-DUST-PREVENTION-INTENSIFIED-SURVEILLANCE (annual ENT + symptom-review для unilateral nasal obstruction / epistaxis / anosmia + low-threshold CT sinus + PPE); альтернатива = OBSERVATION.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Hardwood dust 25y (cabinetmaker) · sinonasal adenocarcinoma surveillance',
        summary_en='52yo cabinetmaker, 25y bespoke furniture workshop (oak/walnut/mahogany hardwood dust, pre-2010s dust-extraction era). IARC Group 1 (Monograph 100C); sinonasal AC latency 30-40y. Engine routes to PreventionPlan: standard = IND-OCC-WOOD-LEATHER-DUST-PREVENTION-INTENSIFIED-SURVEILLANCE (annual ENT + symptom-review for unilateral nasal obstruction / epistaxis / anosmia + low-threshold CT sinus + PPE); alternative = OBSERVATION.',
    ),
    CaseEntry(
        case_id='prevention-peutz-jeghers-phenotype',
        file='patient_pj_classic_phenotype.json',
        label_ua='Peutz-Jeghers classic phenotype · mucocutaneous + intussusception',
        summary_ua='12-річна дівчинка з classic PJ mucocutaneous phenotype (perioral + buccal lentigines + digital-tip pigmentation з раннього дитинства). Index small-bowel intussusception age 9 (jejunal hamartomatous polyp histology). Sibling + paternal aunt similar pigmentation (untested). Engine спрацьовує RF-PEUTZ-JEGHERS-FAMILY-HISTORY-SUSPICION → PreventionPlan: стандарт = counseling + germline STK11 panel; альтернатива = empirical multi-organ PJS surveillance.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Peutz-Jeghers classic phenotype · mucocutaneous + intussusception',
        summary_en='12yo girl with classic PJ mucocutaneous phenotype (perioral + buccal lentigines + digital-tip pigmentation from early childhood). Index small-bowel intussusception age 9 (jejunal hamartomatous polyp histology). Sibling + paternal aunt similar pigmentation (untested). Engine fires RF-PEUTZ-JEGHERS-FAMILY-HISTORY-SUSPICION → PreventionPlan: standard = counseling + germline STK11 panel; alternative = empirical multi-organ PJS surveillance.',
    ),
    CaseEntry(
        case_id='prevention-pms2-carrier',
        file='patient_pms2_carrier_attenuated_phenotype.json',
        label_ua='Lynch PMS2 carrier (lower-penetrance) · CRC ~25-30% + endometrial ~15-20%',
        summary_ua="44-річна жінка, confirmed germline PMS2 c.137G>T (cascade після father's CRC 67, MMR-IHC PMS2-loss). Lower-penetrance Lynch phenotype (PMS2 CRC ~25-30%, endometrial ~15-20%). Engine спрацьовує RF-LYNCH-CONFIRMED-CARRIER → PreventionPlan: стандарт = IND-LYNCH-CARRIER-SURVEILLANCE (colonoscopy q2-3y від 30-35 per NCCN PMS2-specific cadence; endometrial sampling + TVUS q1-2y); aggressive = + CAPP2 aspirin SDM + prophylactic hyst/BSO post-childbearing.",
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Lynch PMS2 carrier (lower-penetrance) · CRC ~25-30% + endometrial ~15-20%',
        summary_en="44yo female, confirmed germline PMS2 c.137G>T (cascade after father's CRC 67, MMR-IHC PMS2-loss). Lower-penetrance Lynch phenotype (PMS2 CRC ~25-30%, endometrial ~15-20%). Engine fires RF-LYNCH-CONFIRMED-CARRIER → PreventionPlan: standard = IND-LYNCH-CARRIER-SURVEILLANCE (colonoscopy q2-3y from 30-35 per NCCN PMS2-specific cadence; endometrial sampling + TVUS q1-2y); aggressive = + CAPP2 aspirin SDM + prophylactic hyst/BSO post-childbearing.",
    ),
    CaseEntry(
        case_id='prevention-prss1-pancreatitis',
        file='patient_hereditary_pancreatitis_prss1.json',
        label_ua='Hereditary chronic pancreatitis (PRSS1) · CAPS EUS/MRCP q1y від 35',
        summary_ua='35-річний чоловік, recurrent acute pancreatitis з 12р, parenchymal atrophy + ductal irregularities on MRCP, confirmed germline PRSS1 patho-variant (autosomal dominant; father + paternal uncle affected, grandfather PDAC 58). EUROPAC PDAC lifetime ~40-50% by 70 (vs ~5% alcoholic CP). Engine спрацьовує RF-CHRONIC-PANCREATITIS-PDAC-PREVENTION → PreventionPlan: CAPS protocol (annual EUS ± MRCP від 40 АБО 20р post-onset) + aggressive smoking-cessation + cascade testing.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Hereditary chronic pancreatitis (PRSS1) · CAPS EUS/MRCP q1y from 35',
        summary_en='35yo male, recurrent acute pancreatitis from 12, parenchymal atrophy + ductal irregularities on MRCP, confirmed germline PRSS1 pathogenic variant (autosomal dominant; father + paternal uncle affected, grandfather PDAC 58). EUROPAC PDAC lifetime ~40-50% by 70 (vs ~5% alcoholic CP). Engine fires RF-CHRONIC-PANCREATITIS-PDAC-PREVENTION → PreventionPlan: CAPS protocol (annual EUS ± MRCP from 40 OR 20y post-onset) + aggressive smoking-cessation + cascade testing.',
    ),
    CaseEntry(
        case_id='prevention-survivor-anthracycline',
        file='patient_anthracycline_late_effect_survivor.json',
        label_ua='Pediatric anthracycline survivor · COG-LTFU cardio surveillance',
        summary_ua='26-річна жінка, 10р post-pediatric Hodgkin ABVD×6 (cumulative doxorubicin ~300 mg/m²). Engine спрацьовує RF-IATROGENIC-ANTHRACYCLINE-LATE-CARDIO-PREVENTION → PreventionPlan: стандарт = echocardiogram + GLS q2-5y per COG-LTFU + ASCO Cardio-Oncology; aggressive = annual echo + cardio-oncology + ARB/β-blocker on subclinical LV dysfunction.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Pediatric anthracycline survivor · COG-LTFU cardio surveillance',
        summary_en='26yo female, 10y post-pediatric Hodgkin ABVD×6 (cumulative doxorubicin ~300 mg/m²). Engine fires RF-IATROGENIC-ANTHRACYCLINE-LATE-CARDIO-PREVENTION → PreventionPlan: standard = echo + GLS q2-5y per COG-LTFU + ASCO Cardio-Oncology; aggressive = annual echo + cardio-oncology + ARB/β-blocker on subclinical LV dysfunction.',
    ),
    CaseEntry(
        case_id='prevention-survivor-cranial-rt',
        file='patient_pediatric_cranial_rt_survivor.json',
        label_ua='Pediatric ALL · 18 Gy PCI · CNS + endocrine + cognitive late effects',
        summary_ua='29-річний чоловік, 22р post-pediatric ALL (вкл. 18 Gy PCI age 7). Hypothyroidism on levothyroxine. Engine спрацьовує RF-IATROGENIC-CRANIAL-RT-LATE-PREVENTION → PreventionPlan: стандарт = COG-LTFU + CCSS + NCCN CNS (annual neuro + symptom-prompted brain MRI; annual endocrine TSH/IGF-1/AM cortisol/gonadal; neurocognitive screening); aggressive = surveillance brain MRI q3y від 30 + endocrinology + neuropsych baseline.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Pediatric ALL · 18 Gy PCI · CNS + endocrine + cognitive late effects',
        summary_en='29yo male, 22y post-pediatric ALL (incl. 18 Gy PCI age 7). Hypothyroidism on levothyroxine. Engine fires RF-IATROGENIC-CRANIAL-RT-LATE-PREVENTION → PreventionPlan: standard = COG-LTFU + CCSS + NCCN CNS (annual neuro + symptom-prompted brain MRI; annual endocrine TSH/IGF-1/AM cortisol/gonadal; neurocognitive screening); aggressive = surveillance brain MRI q3y from 30 + endocrinology + neuropsych baseline.',
    ),
    CaseEntry(
        case_id='prevention-survivor-pediatric-all',
        file='patient_lung_cancer_survivor_anthracycline_pediatric.json',
        label_ua='Pediatric ALL survivor 17y · anthracycline + cranial RT late effects',
        summary_ua='25-річний чоловік, 17р post-pediatric B-ALL (induction + consolidation: doxorubicin ~280 mg/m² + cyclophosphamide + 18 Gy PCI). Multiple iatrogenic RFs: (1) RF-IATROGENIC-ANTHRACYCLINE-LATE-CARDIO-PREVENTION → COG-LTFU echo q2-5y; (2) RF-IATROGENIC-PRIOR-RADIATION-PREVENTION → secondary thyroid/breast/CNS/sarcoma surveillance.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Pediatric ALL survivor 17y · anthracycline + cranial RT late effects',
        summary_en='25yo male, 17y post-pediatric B-ALL (induction + consolidation: doxorubicin ~280 mg/m² + cyclophosphamide + 18 Gy PCI). Multiple iatrogenic RFs: (1) RF-IATROGENIC-ANTHRACYCLINE-LATE-CARDIO-PREVENTION → COG-LTFU echo q2-5y; (2) RF-IATROGENIC-PRIOR-RADIATION-PREVENTION → secondary thyroid/breast/CNS/sarcoma surveillance.',
    ),
    CaseEntry(
        case_id='prevention-universal-age-screening-gap',
        file='patient_age_55_screening_gap.json',
        label_ua='Universal screening gap age 50+ · CRC + mammography + Pap catch-up',
        summary_ua='55-річна жінка без задокументованого age-appropriate скринінгу за останні 5 років (немає CRC modality на файлі, mammogram >7р, немає cervical screening). Engine спрацьовує RF-UNIVERSAL-AGE-50PLUS-SCREENING-GAP → routes to PreventionPlan: comprehensive catch-up (CRC per USPSTF 2021, biennial mammography per USPSTF 2024, hr-HPV q5y per USPSTF 2018).',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Universal screening gap age 50+ · CRC + mammography + Pap catch-up',
        summary_en='55yo female, no documented age-appropriate cancer screening in past 5 years (no CRC modality on file, last mammogram >7y ago, no cervical screening). Engine fires RF-UNIVERSAL-AGE-50PLUS-SCREENING-GAP → PreventionPlan: comprehensive catch-up (CRC per USPSTF 2021, biennial mammography per USPSTF 2024, hr-HPV q5y per USPSTF 2018).',
    ),
    CaseEntry(
        case_id='prevention-universal-current-smoker',
        file='patient_current_smoker_multi_cancer.json',
        label_ua='Current smoker 30 pack-years · cessation + USPSTF LDCT eligibility',
        summary_ua='62-річний чоловік, active 1ppd ×30р (30 pack-years). IARC 100A Group 1 — 17-site risk. Engine спрацьовує RF-UNIVERSAL-CURRENT-SMOKER-MULTI-CANCER → PreventionPlan: (1) structured cessation (varenicline / bupropion / NRT-combo per USPHS/NICE); (2) USPSTF 2021 LDCT eligibility (age 50-80 + ≥20 pack-y + current) → annual LDCT. relevant_diseases=["*"], sidecar documentation.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Current smoker 30 pack-years · cessation + USPSTF LDCT eligibility',
        summary_en='62yo male, active 1ppd ×30y (30 pack-years). IARC 100A Group 1 — 17-site risk. Engine fires RF-UNIVERSAL-CURRENT-SMOKER-MULTI-CANCER → PreventionPlan: (1) structured cessation (varenicline / bupropion / NRT-combo per USPHS/NICE); (2) USPSTF 2021 LDCT eligibility (age 50-80 + ≥20 pack-y + current) → annual LDCT. relevant_diseases=["*"], sidecar documentation.',
    ),
    CaseEntry(
        case_id='prevention-universal-first-degree-family',
        file='patient_first_degree_cancer_family_any.json',
        label_ua='First-degree cancer in family (any) · pedigree + genetic counseling',
        summary_ua='41-річний чоловік, asymptomatic. Father RCC 64, no germline testing. Single first-degree relative, pedigree not yet systematically evaluated. Engine спрацьовує RF-UNIVERSAL-FIRST-DEGREE-CANCER-FAMILY-ANY (pre-triage flag) → PreventionPlan: structured pedigree review (Tyrer-Cuzick / BOADICEA / PREMM5 / MMRpro) + counseling. relevant_diseases=["*"], sidecar documentation only.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='First-degree cancer in family (any) · pedigree + genetic counseling',
        summary_en='41yo male, asymptomatic. Father RCC 64, no germline testing on file. Single first-degree relative, pedigree not systematically evaluated. Engine fires RF-UNIVERSAL-FIRST-DEGREE-CANCER-FAMILY-ANY (pre-triage flag) → PreventionPlan: structured pedigree review (Tyrer-Cuzick / BOADICEA / PREMM5 / MMRpro) + counseling. relevant_diseases=["*"], sidecar documentation only.',
    ),
    CaseEntry(
        case_id='prevention-universal-heavy-alcohol',
        file='patient_heavy_alcohol_multi_cancer.json',
        label_ua='Heavy alcohol use AUDIT-C 9 · UADT/HCC/CRC/breast cluster · naltrexone',
        summary_ua='58-річний чоловік, 4-5 standard drinks/day ×15р, AUDIT-C 9. IARC 100E Group 1 — 7-cancer cluster (UADT, esophageal SCC, HCC, CRC, breast; WCRF probable gastric cardia + pancreas). Engine спрацьовує RF-UNIVERSAL-HEAVY-ALCOHOL-MULTI-CANCER → PreventionPlan: cessation (MI + naltrexone/acamprosate per AUD severity) + UADT exam, LFTs/HCC US+AFP q6mo if cirrhosis, CRC per cadence. relevant_diseases=["*"], sidecar documentation.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Heavy alcohol use AUDIT-C 9 · UADT/HCC/CRC/breast cluster · naltrexone',
        summary_en='58yo male, 4-5 drinks/day ×15y, AUDIT-C 9. IARC 100E Group 1 — 7-cancer cluster (UADT, esophageal SCC, HCC, CRC, breast; WCRF probable gastric cardia + pancreas). Engine fires RF-UNIVERSAL-HEAVY-ALCOHOL-MULTI-CANCER → PreventionPlan: cessation (MI + naltrexone/acamprosate per AUD severity) + UADT exam, LFTs/HCC US+AFP q6mo if cirrhosis, CRC per cadence. relevant_diseases=["*"], sidecar documentation.',
    ),
    CaseEntry(
        case_id='prevention-universal-immunocompromised',
        file='patient_chronic_lymphocytic_immunosuppression.json',
        label_ua='Chronic immunosuppression ≥1y · skin + EBV-VL + Pap + anal screening',
        summary_ua='50-річний чоловік, post-allo-HSCT 7р тому + chronic GVHD на cyclosporine + low-dose prednisone. Engine спрацьовує RF-UNIVERSAL-IMMUNOCOMPROMISED-CANCER-RISK → PreventionPlan: stratified screening per IDSA/ASCO/ACOG/AST (enhanced derm q6-12mo + EBV-VL monitoring + accelerated cervical/anal cytology + HPV catch-up). relevant_diseases=["*"], sidecar documentation.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Chronic immunosuppression ≥1y · skin + EBV-VL + Pap + anal screening',
        summary_en='50yo male, post-allo-HSCT 7y ago + chronic GVHD on cyclosporine + low-dose prednisone. Engine fires RF-UNIVERSAL-IMMUNOCOMPROMISED-CANCER-RISK → PreventionPlan: stratified screening per IDSA/ASCO/ACOG/AST (enhanced derm q6-12mo + EBV-VL monitoring + accelerated cervical/anal cytology + HPV catch-up). relevant_diseases=["*"], sidecar documentation.',
    ),
    CaseEntry(
        case_id='prevention-universal-obesity',
        file='patient_obese_multi_cancer_risk.json',
        label_ua='Obesity BMI 36 + central adiposity · IARC 13-cancer cluster',
        summary_ua='53-річна жінка, BMI 36 (Class II), waist 108 cm. IARC 13-cancer cluster + WCRF/AICR CUP (CRC, postmenopausal breast, endometrium, kidney, HCC priorities). Engine спрацьовує RF-UNIVERSAL-ELEVATED-BMI-CANCER-CLUSTER → PreventionPlan: structured weight-loss (lifestyle + GLP-1 RA / bariatric SDM per BMI) + cancer-cluster screening modification. relevant_diseases=["*"], sidecar documentation only.',
        badge="Prevention Plan", badge_class="bdg-plan", category="prevention",
        label_en='Obesity BMI 36 + central adiposity · IARC 13-cancer cluster',
        summary_en='53yo female, BMI 36 (Class II), waist 108 cm. IARC 13-cancer cluster + WCRF/AICR CUP (CRC, postmenopausal breast, endometrium, kidney, HCC priorities). Engine fires RF-UNIVERSAL-ELEVATED-BMI-CANCER-CLUSTER → PreventionPlan: structured weight-loss (lifestyle + GLP-1 RA / bariatric SDM per BMI) + cancer-cluster screening modification. relevant_diseases=["*"], sidecar documentation only.',
    ),
    # ── /PREVENTION CASES ──
    # ── VERIFIED TREATMENT EXAMPLES (regen via scripts/generate_verified_treatment_examples.py) ──
    CaseEntry(
        case_id="verified-adrenocortical-carcinoma-l1-edp-m-acc",
        file="patient_verified_adrenocortical_carcinoma_l1_edp_m_acc.json",
        label_ua="ADRENOCORTICAL-CARCINOMA · 1L · EDP-M-ACC",
        summary_ua="Перевірений end-to-end приклад для Adrenocortical carcinoma — лінія 1, режим EDP-M-ACC. Автоматично згенеровано з IND-ACC-1L-MITOTANE-EDP; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="ADRENOCORTICAL-CARCINOMA · L1 · EDP-M-ACC",
        summary_en="Verified end-to-end example for Adrenocortical carcinoma — line 1, regimen EDP-M-ACC. Auto-synthesised from IND-ACC-1L-MITOTANE-EDP; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-aitl-l2-belinostat-ptcl",
        file="patient_verified_aitl_l2_belinostat_ptcl.json",
        label_ua="AITL · 2L · BELINOSTAT-PTCL",
        summary_ua="Перевірений end-to-end приклад для Angioimmunoblastic T-Cell Lymphoma — лінія 2, режим BELINOSTAT-PTCL. Автоматично згенеровано з IND-AITL-2L-BELINOSTAT; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="t_cell",
        label_en="AITL · L2 · BELINOSTAT-PTCL",
        summary_en="Verified end-to-end example for Angioimmunoblastic T-Cell Lymphoma — line 2, regimen BELINOSTAT-PTCL. Auto-synthesised from IND-AITL-2L-BELINOSTAT; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-aitl-l2-romidepsin-ptcl",
        file="patient_verified_aitl_l2_romidepsin_ptcl.json",
        label_ua="AITL · 2L · ROMIDEPSIN-PTCL",
        summary_ua="Перевірений end-to-end приклад для Angioimmunoblastic T-Cell Lymphoma — лінія 2, режим ROMIDEPSIN-PTCL. Автоматично згенеровано з IND-AITL-2L-ROMIDEPSIN; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="t_cell",
        label_en="AITL · L2 · ROMIDEPSIN-PTCL",
        summary_en="Verified end-to-end example for Angioimmunoblastic T-Cell Lymphoma — line 2, regimen ROMIDEPSIN-PTCL. Auto-synthesised from IND-AITL-2L-ROMIDEPSIN; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-alcl-l2-crizotinib-alcl",
        file="patient_verified_alcl_l2_crizotinib_alcl.json",
        label_ua="ALCL · 2L · CRIZOTINIB-ALCL",
        summary_ua="Перевірений end-to-end приклад для Anaplastic Large Cell Lymphoma, Systemic — лінія 2, режим CRIZOTINIB-ALCL. Автоматично згенеровано з IND-ALCL-2L-CRIZOTINIB-ALKPOS; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="t_cell",
        label_en="ALCL · L2 · CRIZOTINIB-ALCL",
        summary_en="Verified end-to-end example for Anaplastic Large Cell Lymphoma, Systemic — line 2, regimen CRIZOTINIB-ALCL. Auto-synthesised from IND-ALCL-2L-CRIZOTINIB-ALKPOS; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-aml-l1-aml-7-3-go",
        file="patient_verified_aml_l1_aml_7_3_go.json",
        label_ua="AML · 1L · AML-7-3-GO",
        summary_ua="Перевірений end-to-end приклад для Acute Myeloid Leukemia — лінія 1, режим AML-7-3-GO. Автоматично згенеровано з IND-AML-1L-7-3-GO-CBF; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloid",
        label_en="AML · L1 · AML-7-3-GO",
        summary_en="Verified end-to-end example for Acute Myeloid Leukemia — line 1, regimen AML-7-3-GO. Auto-synthesised from IND-AML-1L-7-3-GO-CBF; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-aml-l2-enasidenib-aml",
        file="patient_verified_aml_l2_enasidenib_aml.json",
        label_ua="AML · 2L · ENASIDENIB-AML",
        summary_ua="Перевірений end-to-end приклад для Acute Myeloid Leukemia — лінія 2, режим ENASIDENIB-AML. Автоматично згенеровано з IND-AML-2L-IDH2-ENASIDENIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloid",
        label_en="AML · L2 · ENASIDENIB-AML",
        summary_en="Verified end-to-end example for Acute Myeloid Leukemia — line 2, regimen ENASIDENIB-AML. Auto-synthesised from IND-AML-2L-IDH2-ENASIDENIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-aml-l2-revumenib-aml",
        file="patient_verified_aml_l2_revumenib_aml.json",
        label_ua="AML · 2L · REVUMENIB-AML",
        summary_ua="Перевірений end-to-end приклад для Acute Myeloid Leukemia — лінія 2, режим REVUMENIB-AML. Автоматично згенеровано з IND-AML-2L-KMT2A-REVUMENIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloid",
        label_en="AML · L2 · REVUMENIB-AML",
        summary_en="Verified end-to-end example for Acute Myeloid Leukemia — line 2, regimen REVUMENIB-AML. Auto-synthesised from IND-AML-2L-KMT2A-REVUMENIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-apl-l1-atra-ato-ida-apl",
        file="patient_verified_apl_l1_atra_ato_ida_apl.json",
        label_ua="APL · 1L · ATRA-ATO-IDA-APL",
        summary_ua="Перевірений end-to-end приклад для Acute Promyelocytic Leukemia — лінія 1, режим ATRA-ATO-IDA-APL. Автоматично згенеровано з IND-APL-1L-ATRA-ATO-IDA; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloid",
        label_en="APL · L1 · ATRA-ATO-IDA-APL",
        summary_en="Verified end-to-end example for Acute Promyelocytic Leukemia — line 1, regimen ATRA-ATO-IDA-APL. Auto-synthesised from IND-APL-1L-ATRA-ATO-IDA; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-b-all-l2-inotuzumab-b-all",
        file="patient_verified_b_all_l2_inotuzumab_b_all.json",
        label_ua="B-ALL · 2L · INOTUZUMAB-B-ALL",
        summary_ua="Перевірений end-to-end приклад для B-Lymphoblastic Leukemia/Lymphoma — лінія 2, режим INOTUZUMAB-B-ALL. Автоматично згенеровано з IND-B-ALL-2L-INOTUZUMAB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="lymphoblastic",
        label_en="B-ALL · L2 · INOTUZUMAB-B-ALL",
        summary_en="Verified end-to-end example for B-Lymphoblastic Leukemia/Lymphoma — line 2, regimen INOTUZUMAB-B-ALL. Auto-synthesised from IND-B-ALL-2L-INOTUZUMAB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-burkitt-l2-rdhap-burkitt",
        file="patient_verified_burkitt_l2_rdhap_burkitt.json",
        label_ua="BURKITT · 2L · RDHAP-BURKITT",
        summary_ua="Перевірений end-to-end приклад для Burkitt Lymphoma — лінія 2, режим RDHAP-BURKITT. Автоматично згенеровано з IND-BURKITT-2L-RDHAP-ASCT; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="b_aggressive",
        label_en="BURKITT · L2 · RDHAP-BURKITT",
        summary_en="Verified end-to-end example for Burkitt Lymphoma — line 2, regimen RDHAP-BURKITT. Auto-synthesised from IND-BURKITT-2L-RDHAP-ASCT; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-cholangiocarcinoma-l2-pemigatinib-cholangio",
        file="patient_verified_cholangiocarcinoma_l2_pemigatinib_cholangio.json",
        label_ua="CHOLANGIOCARCINOMA · 2L · PEMIGATINIB-CHOLANGIO",
        summary_ua="Перевірений end-to-end приклад для Cholangiocarcinoma — лінія 2, режим PEMIGATINIB-CHOLANGIO. Автоматично згенеровано з IND-CHOLANGIO-2L-FGFR2-FUSION-PEMIGATINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="CHOLANGIOCARCINOMA · L2 · PEMIGATINIB-CHOLANGIO",
        summary_en="Verified end-to-end example for Cholangiocarcinoma — line 2, regimen PEMIGATINIB-CHOLANGIO. Auto-synthesised from IND-CHOLANGIO-2L-FGFR2-FUSION-PEMIGATINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-cholangiocarcinoma-l2-zanidatamab",
        file="patient_verified_cholangiocarcinoma_l2_zanidatamab.json",
        label_ua="CHOLANGIOCARCINOMA · 2L · ZANIDATAMAB",
        summary_ua="Перевірений end-to-end приклад для Cholangiocarcinoma — лінія 2, режим ZANIDATAMAB. Автоматично згенеровано з IND-CHOLANGIO-2L-HER2-ZANIDATAMAB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="CHOLANGIOCARCINOMA · L2 · ZANIDATAMAB",
        summary_en="Verified end-to-end example for Cholangiocarcinoma — line 2, regimen ZANIDATAMAB. Auto-synthesised from IND-CHOLANGIO-2L-HER2-ZANIDATAMAB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-cml-l1-2gen-tki-cml",
        file="patient_verified_cml_l1_2gen_tki_cml.json",
        label_ua="CML · 1L · 2GEN-TKI-CML",
        summary_ua="Перевірений end-to-end приклад для Chronic Myeloid Leukemia — лінія 1, режим 2GEN-TKI-CML. Автоматично згенеровано з IND-CML-1L-2GEN-TKI; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloid",
        label_en="CML · L1 · 2GEN-TKI-CML",
        summary_en="Verified end-to-end example for Chronic Myeloid Leukemia — line 1, regimen 2GEN-TKI-CML. Auto-synthesised from IND-CML-1L-2GEN-TKI; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-cml-l2-allohct-cml-advanced",
        file="patient_verified_cml_l2_allohct_cml_advanced.json",
        label_ua="CML · 2L · ALLOHCT-CML-ADVANCED",
        summary_ua="Перевірений end-to-end приклад для Chronic Myeloid Leukemia — лінія 2, режим ALLOHCT-CML-ADVANCED. Автоматично згенеровано з IND-CML-ADVANCED-ALLOHCT; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloid",
        label_en="CML · L2 · ALLOHCT-CML-ADVANCED",
        summary_en="Verified end-to-end example for Chronic Myeloid Leukemia — line 2, regimen ALLOHCT-CML-ADVANCED. Auto-synthesised from IND-CML-ADVANCED-ALLOHCT; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-cml-l2-ponatinib-cml",
        file="patient_verified_cml_l2_ponatinib_cml.json",
        label_ua="CML · 2L · PONATINIB-CML",
        summary_ua="Перевірений end-to-end приклад для Chronic Myeloid Leukemia — лінія 2, режим PONATINIB-CML. Автоматично згенеровано з IND-CML-2L-PONATINIB-T315I; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloid",
        label_en="CML · L2 · PONATINIB-CML",
        summary_en="Verified end-to-end example for Chronic Myeloid Leukemia — line 2, regimen PONATINIB-CML. Auto-synthesised from IND-CML-2L-PONATINIB-T315I; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-crc-l2-tucatinib-trastuzumab-crc",
        file="patient_verified_crc_l2_tucatinib_trastuzumab_crc.json",
        label_ua="CRC · 2L · TUCATINIB-TRASTUZUMAB-CRC",
        summary_ua="Перевірений end-to-end приклад для Colorectal carcinoma — лінія 2, режим TUCATINIB-TRASTUZUMAB-CRC. Автоматично згенеровано з IND-CRC-METASTATIC-2L-HER2-AMP-TUCATINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="CRC · L2 · TUCATINIB-TRASTUZUMAB-CRC",
        summary_en="Verified end-to-end example for Colorectal carcinoma — line 2, regimen TUCATINIB-TRASTUZUMAB-CRC. Auto-synthesised from IND-CRC-METASTATIC-2L-HER2-AMP-TUCATINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-endometrial-l2-pembro-lenvatinib-endom",
        file="patient_verified_endometrial_l2_pembro_lenvatinib_endom.json",
        label_ua="ENDOMETRIAL · 2L · PEMBRO-LENVATINIB-ENDOM",
        summary_ua="Перевірений end-to-end приклад для Endometrial carcinoma — лінія 2, режим PEMBRO-LENVATINIB-ENDOM. Автоматично згенеровано з IND-ENDOMETRIAL-2L-PEMBRO-LENVA-PMMR; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="ENDOMETRIAL · L2 · PEMBRO-LENVATINIB-ENDOM",
        summary_en="Verified end-to-end example for Endometrial carcinoma — line 2, regimen PEMBRO-LENVATINIB-ENDOM. Auto-synthesised from IND-ENDOMETRIAL-2L-PEMBRO-LENVA-PMMR; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-esophageal-l2-tdxd-gastric",
        file="patient_verified_esophageal_l2_tdxd_gastric.json",
        label_ua="ESOPHAGEAL · 2L · TDXD-GASTRIC",
        summary_ua="Перевірений end-to-end приклад для Esophageal carcinoma — лінія 2, режим TDXD-GASTRIC. Автоматично згенеровано з IND-ESOPH-METASTATIC-2L-HER2-POSITIVE-T-DXD; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="ESOPHAGEAL · L2 · TDXD-GASTRIC",
        summary_en="Verified end-to-end example for Esophageal carcinoma — line 2, regimen TDXD-GASTRIC. Auto-synthesised from IND-ESOPH-METASTATIC-2L-HER2-POSITIVE-T-DXD; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-fl-l2-obinutuzumab-benda",
        file="patient_verified_fl_l2_obinutuzumab_benda.json",
        label_ua="FL · 2L · OBINUTUZUMAB-BENDA",
        summary_ua="Перевірений end-to-end приклад для Follicular Lymphoma — лінія 2, режим OBINUTUZUMAB-BENDA. Автоматично згенеровано з IND-FL-2L-OBG-BENDA; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="b_indolent",
        label_en="FL · L2 · OBINUTUZUMAB-BENDA",
        summary_en="Verified end-to-end example for Follicular Lymphoma — line 2, regimen OBINUTUZUMAB-BENDA. Auto-synthesised from IND-FL-2L-OBG-BENDA; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-gastric-l1-bemarituzumab-mfolfox6",
        file="patient_verified_gastric_l1_bemarituzumab_mfolfox6.json",
        label_ua="GASTRIC · 1L · BEMARITUZUMAB-MFOLFOX6",
        summary_ua="Перевірений end-to-end приклад для Gastric and gastroesophageal junction adenocarcinoma — лінія 1, режим BEMARITUZUMAB-MFOLFOX6. Автоматично згенеровано з IND-GASTRIC-METASTATIC-1L-FGFR2B-BEMARITUZUMAB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="GASTRIC · L1 · BEMARITUZUMAB-MFOLFOX6",
        summary_en="Verified end-to-end example for Gastric and gastroesophageal junction adenocarcinoma — line 1, regimen BEMARITUZUMAB-MFOLFOX6. Auto-synthesised from IND-GASTRIC-METASTATIC-1L-FGFR2B-BEMARITUZUMAB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-gastric-l1-flot",
        file="patient_verified_gastric_l1_flot.json",
        label_ua="GASTRIC · 1L · FLOT",
        summary_ua="Перевірений end-to-end приклад для Gastric and gastroesophageal junction adenocarcinoma — лінія 1, режим FLOT. Автоматично згенеровано з IND-GASTRIC-OLIGOMET-SYSTEMIC-PLUS-LOCAL; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="GASTRIC · L1 · FLOT",
        summary_en="Verified end-to-end example for Gastric and gastroesophageal junction adenocarcinoma — line 1, regimen FLOT. Auto-synthesised from IND-GASTRIC-OLIGOMET-SYSTEMIC-PLUS-LOCAL; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-gastric-l2-tdxd-gastric",
        file="patient_verified_gastric_l2_tdxd_gastric.json",
        label_ua="GASTRIC · 2L · TDXD-GASTRIC",
        summary_ua="Перевірений end-to-end приклад для Gastric and gastroesophageal junction adenocarcinoma — лінія 2, режим TDXD-GASTRIC. Автоматично згенеровано з IND-GASTRIC-METASTATIC-2L-HER2-TDXD; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="GASTRIC · L2 · TDXD-GASTRIC",
        summary_en="Verified end-to-end example for Gastric and gastroesophageal junction adenocarcinoma — line 2, regimen TDXD-GASTRIC. Auto-synthesised from IND-GASTRIC-METASTATIC-2L-HER2-TDXD; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-gbm-l2-bevacizumab-gbm",
        file="patient_verified_gbm_l2_bevacizumab_gbm.json",
        label_ua="GBM · 2L · BEVACIZUMAB-GBM",
        summary_ua="Перевірений end-to-end приклад для Glioblastoma — лінія 2, режим BEVACIZUMAB-GBM. Автоматично згенеровано з IND-GBM-RECURRENT-BEVACIZUMAB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="GBM · L2 · BEVACIZUMAB-GBM",
        summary_en="Verified end-to-end example for Glioblastoma — line 2, regimen BEVACIZUMAB-GBM. Auto-synthesised from IND-GBM-RECURRENT-BEVACIZUMAB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-gist-l2-sunitinib-gist-2l",
        file="patient_verified_gist_l2_sunitinib_gist_2l.json",
        label_ua="GIST · 2L · SUNITINIB-GIST-2L",
        summary_ua="Перевірений end-to-end приклад для Gastrointestinal stromal tumor — лінія 2, режим SUNITINIB-GIST-2L. Автоматично згенеровано з IND-GIST-2L-SUNITINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="GIST · L2 · SUNITINIB-GIST-2L",
        summary_en="Verified end-to-end example for Gastrointestinal stromal tumor — line 2, regimen SUNITINIB-GIST-2L. Auto-synthesised from IND-GIST-2L-SUNITINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-hcc-l1-durva-treme-stride",
        file="patient_verified_hcc_l1_durva_treme_stride.json",
        label_ua="HCC · 1L · DURVA-TREME-STRIDE",
        summary_ua="Перевірений end-to-end приклад для Hepatocellular carcinoma — лінія 1, режим DURVA-TREME-STRIDE. Автоматично згенеровано з IND-HCC-SYSTEMIC-1L-DURVA-TREME; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="HCC · L1 · DURVA-TREME-STRIDE",
        summary_en="Verified end-to-end example for Hepatocellular carcinoma — line 1, regimen DURVA-TREME-STRIDE. Auto-synthesised from IND-HCC-SYSTEMIC-1L-DURVA-TREME; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-hcc-l2-cabozantinib-hcc",
        file="patient_verified_hcc_l2_cabozantinib_hcc.json",
        label_ua="HCC · 2L · CABOZANTINIB-HCC",
        summary_ua="Перевірений end-to-end приклад для Hepatocellular carcinoma — лінія 2, режим CABOZANTINIB-HCC. Автоматично згенеровано з IND-HCC-SYSTEMIC-2L-CABOZANTINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="HCC · L2 · CABOZANTINIB-HCC",
        summary_en="Verified end-to-end example for Hepatocellular carcinoma — line 2, regimen CABOZANTINIB-HCC. Auto-synthesised from IND-HCC-SYSTEMIC-2L-CABOZANTINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-hnscc-l2-nivolumab-mono",
        file="patient_verified_hnscc_l2_nivolumab_mono.json",
        label_ua="HNSCC · 2L · NIVOLUMAB-MONO",
        summary_ua="Перевірений end-to-end приклад для Head and neck squamous cell carcinoma — лінія 2, режим NIVOLUMAB-MONO. Автоматично згенеровано з IND-HNSCC-RM-2L-NIVOLUMAB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="HNSCC · L2 · NIVOLUMAB-MONO",
        summary_en="Verified end-to-end example for Head and neck squamous cell carcinoma — line 2, regimen NIVOLUMAB-MONO. Auto-synthesised from IND-HNSCC-RM-2L-NIVOLUMAB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-kaposi-l1-hiv-biktarvy",
        file="patient_verified_kaposi_l1_hiv_biktarvy.json",
        label_ua="KAPOSI · 1L · HIV-BIKTARVY",
        summary_ua="Перевірений end-to-end приклад для Kaposi sarcoma — лінія 1, режим HIV-BIKTARVY. Автоматично згенеровано з IND-KAPOSI-1L-AIDS-ART; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="KAPOSI · L1 · HIV-BIKTARVY",
        summary_en="Verified end-to-end example for Kaposi sarcoma — line 1, regimen HIV-BIKTARVY. Auto-synthesised from IND-KAPOSI-1L-AIDS-ART; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-kaposi-l1-pld-kaposi",
        file="patient_verified_kaposi_l1_pld_kaposi.json",
        label_ua="KAPOSI · 1L · PLD-KAPOSI",
        summary_ua="Перевірений end-to-end приклад для Kaposi sarcoma — лінія 1, режим PLD-KAPOSI. Автоматично згенеровано з IND-KAPOSI-1L-VISCERAL-PLD; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="KAPOSI · L1 · PLD-KAPOSI",
        summary_en="Verified end-to-end example for Kaposi sarcoma — line 1, regimen PLD-KAPOSI. Auto-synthesised from IND-KAPOSI-1L-VISCERAL-PLD; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-kaposi-l1-sirolimus-kaposi-conversion",
        file="patient_verified_kaposi_l1_sirolimus_kaposi_conversion.json",
        label_ua="KAPOSI · 1L · SIROLIMUS-KAPOSI-CONVERSION",
        summary_ua="Перевірений end-to-end приклад для Kaposi sarcoma — лінія 1, режим SIROLIMUS-KAPOSI-CONVERSION. Автоматично згенеровано з IND-KAPOSI-1L-IATROGENIC-MTOR-CONVERSION; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="KAPOSI · L1 · SIROLIMUS-KAPOSI-CONVERSION",
        summary_en="Verified end-to-end example for Kaposi sarcoma — line 1, regimen SIROLIMUS-KAPOSI-CONVERSION. Auto-synthesised from IND-KAPOSI-1L-IATROGENIC-MTOR-CONVERSION; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-laryngeal-l1-cisplatin-concurrent-rt-larynx",
        file="patient_verified_laryngeal_l1_cisplatin_concurrent_rt_larynx.json",
        label_ua="LARYNGEAL · 1L · CISPLATIN-CONCURRENT-RT-LARYNX",
        summary_ua="Перевірений end-to-end приклад для Laryngeal squamous cell carcinoma — лінія 1, режим CISPLATIN-CONCURRENT-RT-LARYNX. Автоматично згенеровано з IND-LARYNGEAL-LA-1L-CRT; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="LARYNGEAL · L1 · CISPLATIN-CONCURRENT-RT-LARYNX",
        summary_en="Verified end-to-end example for Laryngeal squamous cell carcinoma — line 1, regimen CISPLATIN-CONCURRENT-RT-LARYNX. Auto-synthesised from IND-LARYNGEAL-LA-1L-CRT; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-melanoma-l2-imatinib-kit-melanoma",
        file="patient_verified_melanoma_l2_imatinib_kit_melanoma.json",
        label_ua="MELANOMA · 2L · IMATINIB-KIT-MELANOMA",
        summary_ua="Перевірений end-to-end приклад для Cutaneous melanoma — лінія 2, режим IMATINIB-KIT-MELANOMA. Автоматично згенеровано з IND-MELANOMA-2L-KIT-IMATINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="MELANOMA · L2 · IMATINIB-KIT-MELANOMA",
        summary_en="Verified end-to-end example for Cutaneous melanoma — line 2, regimen IMATINIB-KIT-MELANOMA. Auto-synthesised from IND-MELANOMA-2L-KIT-IMATINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-mesothelioma-l2-nivo-mono-mesothelioma-2l",
        file="patient_verified_mesothelioma_l2_nivo_mono_mesothelioma_2l.json",
        label_ua="MESOTHELIOMA · 2L · NIVO-MONO-MESOTHELIOMA-2L",
        summary_ua="Перевірений end-to-end приклад для Malignant mesothelioma — лінія 2, режим NIVO-MONO-MESOTHELIOMA-2L. Автоматично згенеровано з IND-MESOTHELIOMA-2L-NIVO; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="MESOTHELIOMA · L2 · NIVO-MONO-MESOTHELIOMA-2L",
        summary_en="Verified end-to-end example for Malignant mesothelioma — line 2, regimen NIVO-MONO-MESOTHELIOMA-2L. Auto-synthesised from IND-MESOTHELIOMA-2L-NIVO; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-mm-l3-isa-kd-mm-3l",
        file="patient_verified_mm_l3_isa_kd_mm_3l.json",
        label_ua="MM · 3L · ISA-KD-MM-3L",
        summary_ua="Перевірений end-to-end приклад для Multiple Myeloma — лінія 3, режим ISA-KD-MM-3L. Автоматично згенеровано з IND-MM-3L-ISA-KD; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloma",
        label_en="MM · L3 · ISA-KD-MM-3L",
        summary_en="Verified end-to-end example for Multiple Myeloma — line 3, regimen ISA-KD-MM-3L. Auto-synthesised from IND-MM-3L-ISA-KD; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-mtc-l1-cabozantinib-mtc-1l",
        file="patient_verified_mtc_l1_cabozantinib_mtc_1l.json",
        label_ua="MTC · 1L · CABOZANTINIB-MTC-1L",
        summary_ua="Перевірений end-to-end приклад для Medullary thyroid carcinoma — лінія 1, режим CABOZANTINIB-MTC-1L. Автоматично згенеровано з IND-MTC-ADVANCED-1L-CABOZANTINIB-RET-WT; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="MTC · L1 · CABOZANTINIB-MTC-1L",
        summary_en="Verified end-to-end example for Medullary thyroid carcinoma — line 1, regimen CABOZANTINIB-MTC-1L. Auto-synthesised from IND-MTC-ADVANCED-1L-CABOZANTINIB-RET-WT; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-npc-l1-npc-gc-induction-ccrt",
        file="patient_verified_npc_l1_npc_gc_induction_ccrt.json",
        label_ua="NPC · 1L · NPC-GC-INDUCTION-CCRT",
        summary_ua="Перевірений end-to-end приклад для Nasopharyngeal carcinoma — лінія 1, режим NPC-GC-INDUCTION-CCRT. Автоматично згенеровано з IND-NPC-1L-LOCALLY-ADVANCED-CCRT; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NPC · L1 · NPC-GC-INDUCTION-CCRT",
        summary_en="Verified end-to-end example for Nasopharyngeal carcinoma — line 1, regimen NPC-GC-INDUCTION-CCRT. Auto-synthesised from IND-NPC-1L-LOCALLY-ADVANCED-CCRT; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-npc-l1-npc-gc-toripalimab",
        file="patient_verified_npc_l1_npc_gc_toripalimab.json",
        label_ua="NPC · 1L · NPC-GC-TORIPALIMAB",
        summary_ua="Перевірений end-to-end приклад для Nasopharyngeal carcinoma — лінія 1, режим NPC-GC-TORIPALIMAB. Автоматично згенеровано з IND-NPC-1L-METASTATIC-GC-PD1; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NPC · L1 · NPC-GC-TORIPALIMAB",
        summary_en="Verified end-to-end example for Nasopharyngeal carcinoma — line 1, regimen NPC-GC-TORIPALIMAB. Auto-synthesised from IND-NPC-1L-METASTATIC-GC-PD1; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-nsclc-l1-capmatinib-nsclc",
        file="patient_verified_nsclc_l1_capmatinib_nsclc.json",
        label_ua="NSCLC · 1L · CAPMATINIB-NSCLC",
        summary_ua="Перевірений end-to-end приклад для Non-small cell lung cancer — лінія 1, режим CAPMATINIB-NSCLC. Автоматично згенеровано з IND-NSCLC-MET-EX14-1L-CAPMATINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NSCLC · L1 · CAPMATINIB-NSCLC",
        summary_en="Verified end-to-end example for Non-small cell lung cancer — line 1, regimen CAPMATINIB-NSCLC. Auto-synthesised from IND-NSCLC-MET-EX14-1L-CAPMATINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-nsclc-l1-larotrectinib-pantumor",
        file="patient_verified_nsclc_l1_larotrectinib_pantumor.json",
        label_ua="NSCLC · 1L · LAROTRECTINIB-PANTUMOR",
        summary_ua="Перевірений end-to-end приклад для Non-small cell lung cancer — лінія 1, режим LAROTRECTINIB-PANTUMOR. Автоматично згенеровано з IND-NSCLC-NTRK-FUSION-1L-LAROTRECTINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NSCLC · L1 · LAROTRECTINIB-PANTUMOR",
        summary_en="Verified end-to-end example for Non-small cell lung cancer — line 1, regimen LAROTRECTINIB-PANTUMOR. Auto-synthesised from IND-NSCLC-NTRK-FUSION-1L-LAROTRECTINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-nsclc-l1-selpercatinib-nsclc",
        file="patient_verified_nsclc_l1_selpercatinib_nsclc.json",
        label_ua="NSCLC · 1L · SELPERCATINIB-NSCLC",
        summary_ua="Перевірений end-to-end приклад для Non-small cell lung cancer — лінія 1, режим SELPERCATINIB-NSCLC. Автоматично згенеровано з IND-NSCLC-RET-FUSION-1L-SELPERCATINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NSCLC · L1 · SELPERCATINIB-NSCLC",
        summary_en="Verified end-to-end example for Non-small cell lung cancer — line 1, regimen SELPERCATINIB-NSCLC. Auto-synthesised from IND-NSCLC-RET-FUSION-1L-SELPERCATINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-nsclc-l2-amivantamab-mono-nsclc-ex20ins",
        file="patient_verified_nsclc_l2_amivantamab_mono_nsclc_ex20ins.json",
        label_ua="NSCLC · 2L · AMIVANTAMAB-MONO-NSCLC-EX20INS",
        summary_ua="Перевірений end-to-end приклад для Non-small cell lung cancer — лінія 2, режим AMIVANTAMAB-MONO-NSCLC-EX20INS. Автоматично згенеровано з IND-NSCLC-2L-EGFR-EX20INS-AMIVANTAMAB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NSCLC · L2 · AMIVANTAMAB-MONO-NSCLC-EX20INS",
        summary_en="Verified end-to-end example for Non-small cell lung cancer — line 2, regimen AMIVANTAMAB-MONO-NSCLC-EX20INS. Auto-synthesised from IND-NSCLC-2L-EGFR-EX20INS-AMIVANTAMAB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-nsclc-l2-docetaxel-ramucirumab",
        file="patient_verified_nsclc_l2_docetaxel_ramucirumab.json",
        label_ua="NSCLC · 2L · DOCETAXEL-RAMUCIRUMAB",
        summary_ua="Перевірений end-to-end приклад для Non-small cell lung cancer — лінія 2, режим DOCETAXEL-RAMUCIRUMAB. Автоматично згенеровано з IND-NSCLC-2L-PD-L1-POST-IO-DOCETAXEL; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NSCLC · L2 · DOCETAXEL-RAMUCIRUMAB",
        summary_en="Verified end-to-end example for Non-small cell lung cancer — line 2, regimen DOCETAXEL-RAMUCIRUMAB. Auto-synthesised from IND-NSCLC-2L-PD-L1-POST-IO-DOCETAXEL; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-nsclc-l2-larotrectinib-pantumor",
        file="patient_verified_nsclc_l2_larotrectinib_pantumor.json",
        label_ua="NSCLC · 2L · LAROTRECTINIB-PANTUMOR",
        summary_ua="Перевірений end-to-end приклад для Non-small cell lung cancer — лінія 2, режим LAROTRECTINIB-PANTUMOR. Автоматично згенеровано з IND-NSCLC-2L-NTRK-LAROTRECTINIB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NSCLC · L2 · LAROTRECTINIB-PANTUMOR",
        summary_en="Verified end-to-end example for Non-small cell lung cancer — line 2, regimen LAROTRECTINIB-PANTUMOR. Auto-synthesised from IND-NSCLC-2L-NTRK-LAROTRECTINIB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-nsclc-l2-t-dxd-nsclc",
        file="patient_verified_nsclc_l2_t_dxd_nsclc.json",
        label_ua="NSCLC · 2L · T-DXD-NSCLC",
        summary_ua="Перевірений end-to-end приклад для Non-small cell lung cancer — лінія 2, режим T-DXD-NSCLC. Автоматично згенеровано з IND-NSCLC-2L-HER2-MUT-T-DXD; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="NSCLC · L2 · T-DXD-NSCLC",
        summary_en="Verified end-to-end example for Non-small cell lung cancer — line 2, regimen T-DXD-NSCLC. Auto-synthesised from IND-NSCLC-2L-HER2-MUT-T-DXD; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-ovarian-l2-mirvetuximab-ovarian",
        file="patient_verified_ovarian_l2_mirvetuximab_ovarian.json",
        label_ua="OVARIAN · 2L · MIRVETUXIMAB-OVARIAN",
        summary_ua="Перевірений end-to-end приклад для Ovarian carcinoma — лінія 2, режим MIRVETUXIMAB-OVARIAN. Автоматично згенеровано з IND-OVARIAN-2L-PLAT-RES-MIRVETUXIMAB; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="OVARIAN · L2 · MIRVETUXIMAB-OVARIAN",
        summary_en="Verified end-to-end example for Ovarian carcinoma — line 2, regimen MIRVETUXIMAB-OVARIAN. Auto-synthesised from IND-OVARIAN-2L-PLAT-RES-MIRVETUXIMAB; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-pdac-l2-nal-iri-5fu-lv-pdac",
        file="patient_verified_pdac_l2_nal_iri_5fu_lv_pdac.json",
        label_ua="PDAC · 2L · NAL-IRI-5FU-LV-PDAC",
        summary_ua="Перевірений end-to-end приклад для Pancreatic ductal adenocarcinoma — лінія 2, режим NAL-IRI-5FU-LV-PDAC. Автоматично згенеровано з IND-PDAC-METASTATIC-2L-NAL-IRI; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="PDAC · L2 · NAL-IRI-5FU-LV-PDAC",
        summary_en="Verified end-to-end example for Pancreatic ductal adenocarcinoma — line 2, regimen NAL-IRI-5FU-LV-PDAC. Auto-synthesised from IND-PDAC-METASTATIC-2L-NAL-IRI; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-pheochromocytoma-l1-alpha-blockade-preop-pheo",
        file="patient_verified_pheochromocytoma_l1_alpha_blockade_preop_pheo.json",
        label_ua="PHEOCHROMOCYTOMA · 1L · ALPHA-BLOCKADE-PREOP-PHEO",
        summary_ua="Перевірений end-to-end приклад для Pheochromocytoma — лінія 1, режим ALPHA-BLOCKADE-PREOP-PHEO. Автоматично згенеровано з IND-PHEO-LOCALIZED-1L-ALPHA-BLOCKADE-ADRENALECTOMY; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="PHEOCHROMOCYTOMA · L1 · ALPHA-BLOCKADE-PREOP-PHEO",
        summary_en="Verified end-to-end example for Pheochromocytoma — line 1, regimen ALPHA-BLOCKADE-PREOP-PHEO. Auto-synthesised from IND-PHEO-LOCALIZED-1L-ALPHA-BLOCKADE-ADRENALECTOMY; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-pituitary-adenoma-l1-cabergoline-prolactinoma",
        file="patient_verified_pituitary_adenoma_l1_cabergoline_prolactinoma.json",
        label_ua="PITUITARY-ADENOMA · 1L · CABERGOLINE-PROLACTINOMA",
        summary_ua="Перевірений end-to-end приклад для Pituitary adenoma — лінія 1, режим CABERGOLINE-PROLACTINOMA. Автоматично згенеровано з IND-PITUITARY-ADENOMA-1L-PROLACTINOMA-CABERGOLINE; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="PITUITARY-ADENOMA · L1 · CABERGOLINE-PROLACTINOMA",
        summary_en="Verified end-to-end example for Pituitary adenoma — line 1, regimen CABERGOLINE-PROLACTINOMA. Auto-synthesised from IND-PITUITARY-ADENOMA-1L-PROLACTINOMA-CABERGOLINE; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-pituitary-adenoma-l1-pasireotide-cushing",
        file="patient_verified_pituitary_adenoma_l1_pasireotide_cushing.json",
        label_ua="PITUITARY-ADENOMA · 1L · PASIREOTIDE-CUSHING",
        summary_ua="Перевірений end-to-end приклад для Pituitary adenoma — лінія 1, режим PASIREOTIDE-CUSHING. Автоматично згенеровано з IND-PITUITARY-ADENOMA-1L-CUSHING-SURGERY-MEDICAL; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="PITUITARY-ADENOMA · L1 · PASIREOTIDE-CUSHING",
        summary_en="Verified end-to-end example for Pituitary adenoma — line 1, regimen PASIREOTIDE-CUSHING. Auto-synthesised from IND-PITUITARY-ADENOMA-1L-CUSHING-SURGERY-MEDICAL; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-pituitary-adenoma-l1-ssa-acromegaly",
        file="patient_verified_pituitary_adenoma_l1_ssa_acromegaly.json",
        label_ua="PITUITARY-ADENOMA · 1L · SSA-ACROMEGALY",
        summary_ua="Перевірений end-to-end приклад для Pituitary adenoma — лінія 1, режим SSA-ACROMEGALY. Автоматично згенеровано з IND-PITUITARY-ADENOMA-1L-ACROMEGALY-SURGERY-SSA; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="PITUITARY-ADENOMA · L1 · SSA-ACROMEGALY",
        summary_en="Verified end-to-end example for Pituitary adenoma — line 1, regimen SSA-ACROMEGALY. Auto-synthesised from IND-PITUITARY-ADENOMA-1L-ACROMEGALY-SURGERY-SSA; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-prostate-l2-cabazitaxel-mcrpc",
        file="patient_verified_prostate_l2_cabazitaxel_mcrpc.json",
        label_ua="PROSTATE · 2L · CABAZITAXEL-MCRPC",
        summary_ua="Перевірений end-to-end приклад для Prostate adenocarcinoma — лінія 2, режим CABAZITAXEL-MCRPC. Автоматично згенеровано з IND-PROSTATE-MCRPC-2L-CABAZITAXEL; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="PROSTATE · L2 · CABAZITAXEL-MCRPC",
        summary_en="Verified end-to-end example for Prostate adenocarcinoma — line 2, regimen CABAZITAXEL-MCRPC. Auto-synthesised from IND-PROSTATE-MCRPC-2L-CABAZITAXEL; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-sclc-l2-topotecan-sclc-rr",
        file="patient_verified_sclc_l2_topotecan_sclc_rr.json",
        label_ua="SCLC · 2L · TOPOTECAN-SCLC-RR",
        summary_ua="Перевірений end-to-end приклад для Small cell lung cancer — лінія 2, режим TOPOTECAN-SCLC-RR. Автоматично згенеровано з IND-SCLC-RR-TOPOTECAN; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="SCLC · L2 · TOPOTECAN-SCLC-RR",
        summary_en="Verified end-to-end example for Small cell lung cancer — line 2, regimen TOPOTECAN-SCLC-RR. Auto-synthesised from IND-SCLC-RR-TOPOTECAN; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-soft-tissue-sarcoma-l2-gemcitabine-docetaxel-sts",
        file="patient_verified_soft_tissue_sarcoma_l2_gemcitabine_docetaxel_sts.json",
        label_ua="SOFT-TISSUE-SARCOMA · 2L · GEMCITABINE-DOCETAXEL-STS",
        summary_ua="Перевірений end-to-end приклад для Soft tissue sarcoma — лінія 2, режим GEMCITABINE-DOCETAXEL-STS. Автоматично згенеровано з IND-STS-ADVANCED-2L-GEMDOC; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="SOFT-TISSUE-SARCOMA · L2 · GEMCITABINE-DOCETAXEL-STS",
        summary_en="Verified end-to-end example for Soft tissue sarcoma — line 2, regimen GEMCITABINE-DOCETAXEL-STS. Auto-synthesised from IND-STS-ADVANCED-2L-GEMDOC; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-urothelial-l2-ev-mono-urothelial",
        file="patient_verified_urothelial_l2_ev_mono_urothelial.json",
        label_ua="UROTHELIAL · 2L · EV-MONO-UROTHELIAL",
        summary_ua="Перевірений end-to-end приклад для Urothelial carcinoma — лінія 2, режим EV-MONO-UROTHELIAL. Автоматично згенеровано з IND-UROTHELIAL-2L-EV; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="solid",
        label_en="UROTHELIAL · L2 · EV-MONO-UROTHELIAL",
        summary_en="Verified end-to-end example for Urothelial carcinoma — line 2, regimen EV-MONO-UROTHELIAL. Auto-synthesised from IND-UROTHELIAL-2L-EV; engine routes to it as the default plan.",
    ),
    CaseEntry(
        case_id="verified-wm-l2-vrd-wm",
        file="patient_verified_wm_l2_vrd_wm.json",
        label_ua="WM · 2L · VRD-WM",
        summary_ua="Перевірений end-to-end приклад для Waldenström Macroglobulinemia / Lymphoplasmacytic Lymphoma — лінія 2, режим VRD-WM. Автоматично згенеровано з IND-WM-2L-VRD; engine обирає його як план за замовчуванням.",
        badge="Treatment Plan", badge_class="bdg-plan", category="myeloma",
        label_en="WM · L2 · VRD-WM",
        summary_en="Verified end-to-end example for Waldenström Macroglobulinemia / Lymphoplasmacytic Lymphoma — line 2, regimen VRD-WM. Auto-synthesised from IND-WM-2L-VRD; engine routes to it as the default plan.",
    ),
    # ── /VERIFIED TREATMENT EXAMPLES ──

]


# Normalize legacy rows once at import time.  Existing hand-authored rows
# predate structured example metadata; generated BMA rows set the fields
# explicitly.  This preserves their labels while making presentation and
# publication decisions data-driven rather than filename-driven.
for _case in CASES:
    if _case.case_id in GALLERY_FEATURED_CASE_IDS:
        _case.quality_tier = "showcase"
    elif _case.badge == "Diagnostic Brief":
        _case.quality_tier = "diagnostic"
        _case.scenario_type = "diagnostic"
    elif _case.badge in {"BMA", "Molecular example"}:
        _case.quality_tier = "molecular"
        _case.scenario_type = "molecular"
    elif _case.badge == "Prevention Plan":
        _case.scenario_type = "prevention"

    if _case.badge in {"Auto-stub", "Variant"} or _case.case_id in QUARANTINED_CASE_IDS:
        _case.visibility = "internal"
