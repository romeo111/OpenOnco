...
.about-purpose {
  ...
  border-radius: 12px;
  background: linear-gradient(125deg, #f0fdf4 0%, #ffffff 72%);
}
.about-purpose h2,
...
.about-purpose p:not(.about-eyebrow) {
  max-width: 760px;
  color: var(--gray-700);
}
.about-purpose p + p { margin-top: 12px; }
.about-commitment {
  align-self: stretch;
  padding: 20px;
  border-radius: 8px;
...
.about-commitment .about-eyebrow { color: #a7f3d0; }
.about-commitment p:last-child {
  color: white;
  font: 600 20px/1.3 var(--font-display);
}
.about-work,
.about-effort { margin: 0 0 40px; }
...
.about-effort-grid article:nth-child(2) { border-top-color: var(--amber); }
.about-effort-grid article:nth-child(3) { border-top-color: var(--red); }
.about-action {
  display: flex;
  align-items: center;
...
.about-action h2 { color: white; font-size: 28px; margin-bottom: 6px; }
.about-action p:not(.about-eyebrow) { color: rgba(255,255,255,.78); }
...
@media (max-width: 920px) {
  ...
  .about-purpose { gap: 16px; }
  .about-step,
  .about-effort-grid article { min-height: 0; }
  .about-step span { margin-bottom: 16px; }
...
@media (max-width: 700px) {
  ...
  .about-purpose,
  .about-effort,
  .about-action { padding: 20px; }
  .about-purpose h2,
  .about-section-heading h2 { font-size: 26px; }
  .about-action h2 { font-size: 25px; }
}
...
.gallery-hero {
  ...
  color: white;
  background:
    radial-gradient(circle at 88% 0%, rgba(167, 243, 208, .22), transparent 34%),
    linear-gradient(132deg, #0a2e1a 0%, #14532d 100%);
  box-shadow: 0 12px 28px rgba(10, 46, 26, .16);
}
.gallery-eyebrow {
  margin: 0 0 8px;
  color: #a7f3d0;
...
.gallery-safety-note a {
  color: white;
  font-weight: 600;
}
.showcase-section { margin: 34px 0 28px; }
...
.gallery-try-link:hover { color: var(--green-900); text-decoration: underline; }
.showcase-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 14px;
...
.showcase-card {
  ...
  border-radius: 12px;
  background: linear-gradient(145deg, #f7fdf9, #ecfdf5);
  color: var(--gray-900);
  text-decoration: none;
...
.showcase-card::after {
  ...
  width: 130px;
  height: 130px;
  border: 22px solid rgba(22, 101, 52, .08);
  border-radius: 50%;
  content: "";
}
.showcase-card:hover {
  border-color: var(--green-600);
  box-shadow: 0 10px 24px rgba(10, 46, 26, .11);
...
.showcase-card-kind,
.showcase-card-link { position: relative; z-index: 1; }
.showcase-card-number {
  display: inline-block;
  margin-right: 8px;
  color: var(--green-700);
...
.gallery-explainer li strong { color: var(--green-900); }
.gallery-explainer li span {
  color: var(--gray-700);
  font-family: var(--font-sans);
  font-size: 13px;
  font-weight: 400;
...
@media (max-width: 700px) {
  ...
  .gallery-hero { padding: 26px 22px; border-radius: 12px; }
  .gallery-hero .lead { font-size: 16px; }
  .gallery-section-heading { align-items: flex-start; flex-direction: column; }
  .showcase-grid { grid-template-columns: 1fr; }
  .showcase-card { min-height: 0; padding: 21px; }
  .gallery-explainer { padding: 20px; }
  .gallery-explainer ol { grid-template-columns: 1fr; gap: 12px; }
}
...
.quest-toolbar > .qt-label:nth-of-type(-n+2) {
  ...
  /* Reserve the full field-group height so headings line up even when one
     group has an additional search control. */
  min-height: 120px;
}
.quest-toolbar > .qt-label:nth-of-type(-n+2) {
...
.quest-toolbar > .qt-label:nth-of-type(-n+2) select {
  ...
  background: white;
  cursor: pointer;
  /* Keep the primary selects on one baseline within their equal-height groups. */
  margin-top: auto;
...
.quest-toolbar > .qt-reset {
  /* Actions remain aligned with the primary select controls. */
  align-self: flex-end;
}
.qt-modes {
