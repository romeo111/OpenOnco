...
- skill-orient framing цього не змінює — voнo посилює explicit-ність
...
Behaviorально identical to сьогоднішній output, але організація per-domain.
...
- `shared/` — domain-agnostic (rendering pipeline не імпортує doменні)
...
- Subspecialty separation тільки коли rules вiдрізняються істотно
...
entities що підтверджують **kожен rule** у скілі. CI gate: skill
...
Skills mають оголошувати applicability в кожному mode:
```python
